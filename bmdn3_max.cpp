...
ConfigFileText configfilefext1[128]; //need fill at setting config file
...
for(unsigned i=0; i<SavedTxtFilesVal; i++)
modemp[im].SSC2(ScanTxtFilesNum[i]);
...
// сброс кодека
	...
	outportb(0x0, 0x02); for(i=0; i<sizeof(ab10); i++) outportb(0x0F, ab10[i]); outportb(0x10, 0x01); for(i=0; i<200*sizeof(ab10); i++);
...
//			Счётчик ошибок после комбинатора	16 бит
  ...
  unStatePUMp[mod].statePUM.ulErrOut 					 	+=  modemp[deviceindex].protectionRxBytesFromProtection;
  unStatePUM.statePUM.ulErrOut 					 	+=  modemp[deviceindex].protectionRxBytesFromProtection;
...
//			Счётчик ошибок не исправленных РС в 2 стволе
  ...
  unStatePUMp[mod].statePUM.ulErrAfterRS2 				  +=	 modemp[deviceindex+1].fecUncorrectedBlockCounter;
...
//#endif
//___________________________________________________
 PVG610_API_NetE1AlarmsGet(deviceindex,0, 0x1fffffl, &Stat[0]);
...
#define UPDATE_ACM_COUNTERS_PERIOD	(10)
 ...
 unsigned char deviceindex;
  ...
  for(unsigned i = 0; i < MODEM_QUANTITY; i++)
  {
   ...
   if(modemp[deviceindex].acquireStatus == LOCK_OK)
   {
	...
	modemp[deviceindex].TimesTryGetLock = 0;
   }
   else
   {
