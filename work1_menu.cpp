...
SatSet4.Init( &theAnyStrings.c6d[0],
#else
SatSet4.Init( &theAnyStrings.c7d[0],  //tr freq
//SatSet4.Init( "%07d",  //tr freq  
#endif
  				  8, 2,  //x,y
				  1 ,		//  changable
	   			  ZeroStorage, //sweep in storage - new
...
#ifndef PROG_PU_MSAT5
SatSet5.Init( &theAnyStrings.c6d[0],
#else
SatSet5.Init( &theAnyStrings.c7d[0],  //tr freq
//SatSet4.Init( "%07d",  //tr freq  
#endif
  				  8, 2,  //x,y
				  1 ,		//  changable
...
#else
					&theAnyStrings.cSatSoundV[0],	 //offon
#endif
					0, //init value
#ifndef PROG_PU_MSAT5	
					6, //chars in one on indikator
...
#ifndef RECEIVE_MOD_LOCK
    &SatTestModeItem,  &SatSoundItem,  &SatSystemItem, NULL,	//140227

 //   &SatTimingItem, &SatSoundItem, &SatSystemItem, NULL,	//130412
#else
	  &SatTestModeItem,  &SatExtLockItem,  &SatSystemItem, NULL,	//140227
...
#else
    &SatTimingItem, &SatExtLockPItem, &SatSystemItem, NULL,	//130412
#endif
 ...
 //   &SatTimingItem, &SatTFItem, &SatSystemItem, NULL,
  //140227  &SatTimingItem, &SatLowPItem, &SatSystemItem, NULL,	
	...
	&SatSet3Item, &SatTFItem, &SatSystemItem, NULL,	
	 ...
	 &SatTimingItem, &SatLowPItem, &SatSystemItem, NULL,	
#endif
...
extern "C" unsigned char ReadDevId_utest(unsigned char flag_lock)
{
...
check_ver = ( ((unsigned long)Buf[485] << 24) + ((unsigned long)Buf[486] << 16) 
 ...
 if(id_crc == Buf[DEV_ID_SIZE - 1])	
 {
    flag_crc = 1;
  //	printfp("\n\r1");
   if(start_main_loop) printfp("\n\r ReadDevId storage 1 checksum ok!");	
 }
 else
 {
    if(start_main_loop) printfp("\n\r ReadDevId storage 1 checksum wrong!");	
 }
...
check_ver = ( ((unsigned long)Buf[485] << 24) + ((unsigned long)Buf[486] << 16) 
 ...
 if(id_crc == Buf[DEV_ID_SIZE - 1])	
 {
    flag_crc = 1;
  //	printfp("\n\r1");
    if(start_main_loop) printfp("\n\r ReadDevId storage 2 checksum ok!");	
 }
...
//  read_dataflash (ID_ADDRESS, DEV_ID_SIZE, (char *)&Buf[0]); //150120
     if(start_main_loop) printfp("\n\r ReadDevId storage 2 checksum wrong!");	
...
//130409		strcpy(cSatSet6   ,"  Transmit Bound (MHz): ");
...
#else
		strcpy(cSatTiming,   "   Spectrum Invert      ");
#endif
		strcpy(cSatSound,    "        Alarm:          ");
 ...
 //171027		    strcpy(cSatTestModeIV,	   "M  MT S  ST M  MT S  ST M -MT-S -ST-M +MT+S +ST+");
		            strcpy(cSatTestModeIV,	   "    T     T     T     T   - T-  - T-  + T+  + T+");//171027 remove master / slave
...
//		strcpy(cSatInputV,	 "Available   Lost   ");
   ...
   //t		strcpy(cEmpty,     "                ");
		...
		strcpy(cTestSignalData,	   "ÂûêëBARSL1  L2  L3  L4  ");
		strcpy(cRemTestSignalData,  "C8");
		strcpy(cSoundData,	   "2Ìîíî4Ìîíî");
