...
/*
#ifdef GET_CONFIG_INFO

#define IS_WRITED_SIZE (8)

class WrittenFiles
{
unsigned char IsWrited[8];
unsigned short CheckSum;
void Control();
void CalcCheckSum();
void AddUnit(unsigned char);
void SetData(unsigned char, unsigned char);
};

void WrittenFiles::Control(void)
{
unsigned short chs = 1;
for(long i = 0; i <IS_WRITED_SIZE ;i++)
{
  chs += IsWrited[i];
} 
  if(chs != CheckSum) 
...
void WrittenFiles::CalcCheckSum(void)
{
unsigned short chs = 1;
for(long i = 0; i <IS_WRITED_SIZE ;i++)
{
...
void WrittenFiles::AddUnit(unsigned char unit)
{
unsigned char index = unit>>3; // divide to 8
IsWrited[index] |= 1<<(index%8);
CalcCheckSum();
WriteDevId();
}

void WrittenFiles::SetData(unsigned char Data, unsigned char index)
{
if(index < IS_WRITED_SIZE)
{
IsWrited[index] = Data;
}
if(index == IS_WRITED_SIZE)
...
// {	
 ...
 // printfpd(" %d ", provnum);     
 // printfpd(" %X", uProt[provnum].protection.ProtTx );     
 // printfpd(" %X",  uProt[provnum].protection.PhyIDTx + ID_CORR);     
...
// {	
  //   printfp("\n\r PVG610_API_ProtectionRxModeSet");     
   //  printfpd(" %d", provnum);     
   //  printfpd(" %X",  uProt[provnum].protection.ProtRx);     
   //  printfpd(" %X ", uProt[provnum].protection.PhyIDRx+ ID_CORR);     
...
//  unsigned char ConfigTxtData[TXT_REAL_LEN];
 ...
 /*
#define ACM_MODES (16)
//#define MAX_NUM_CFG	 (16)	 /// MAX не более 16 cfg иначе не линкуется
//#define MAX_NUM_CFG	 (32)	 /// MAX не более 16 cfg иначе не линкуется
#define MAX_NUM_CFG	 (45)	 /// MAX не более 16 cfg иначе не линкуется
 */



//#define MAX_NUM_CFG	 (MAX_CFG_FILE_VARIANT)	 //test
...
//unsigned char havestm = CheckIsOnStm1();
//if(havestm) GetOnLaser();
//_____________________________________________________

//_____________________________________________________set gigabit ethernet
...
void SSC1(unsigned char variant) //old
{
  printfp("\n\rSSC1");
...
//}
 ...
 configfilefext.LengthTxt =  length;
 itxt = 0;
for(unsigned long its = FULL_HEADER_SIZE + 4; its < (FULL_HEADER_SIZE + length + 4); its++)
{
   configfilefext.ConfigTxtData.Data[itxt++] = TFTP_Buffer[its];
   printfpd(" %c", TFTP_Buffer[its]);
}
...
modemp[im].SSC1(variant);
 /* bmdn4 - do not need
cod = (((modemp[im].configfilefext.ConfigTxtData.emb4md15504cfg.rateSymbol / 1000000l) * 70 + 5 ) / 100) - 1;
if(cod>MAXIMUM_COD) cod = MAXIMUM_COD;
outportb(BLOCK_SWEEP1+ PORT_FILTER*((im>>1) + 1), cod);		//
   */
//printEthLongHex((im>>1) + 1);
//printEthLongHex(modemp[im].configfilefext.ConfigTxtData.emb4md15504cfg.rateSymbol);

//if(modemp[im].CheckIsOnStm1())
//if(GetStmPhyReg(im) & TX_FAULT_S) //laser modul absend
//{
// SetOptics(im, 0);
//}
//else
...
/* bmdn4 -do not need
cod = (((modemp[flagm].configfilefext.ConfigTxtData.emb4md15504cfg.rateSymbol / 1000000l) * 70 + 5 ) / 100) - 1;
if(cod>MAXIMUM_COD) cod = MAXIMUM_COD;
outportb(BLOCK_SWEEP1+ PORT_FILTER*((flagm>>1)+1), cod);	
   */
...
//if(GetStmPhyReg(flagm) & TX_FAULT_S) //laser modul absend
//{
// SetOptics(flagm, 0);
//}
//else
...
void HotStartConfigFile1(unsigned char flagm)
{
// char ret;
//delay_mcs(5000000l); //for test !!!!!!
#ifndef USART0_TRY_PDC_1T
...
//printf("\n\n\n\n\n\n\n\n\r_____________________%d", im);
//hotstartconfigfile FirmwareLoadm(flagm);
//wd_reset();
//}
ConfigFileLoadm(GetNumConfigFile(flagm), flagm);
//delay_mcs(5000l);
...
ConfigFileRunm(flagm);
 //delay_mcs(5000l);
//hotstartconfigfile ModemAlarmEnableSetAllm(flagm);
//SetModemXILNPL1m(currentrezerv); //before enover xilinxes	- maked in funktion
//hotstartconfigfile SetModemXILNPL1m(flagm);
...
#ifdef ETH_TRANZIT
//printEthLongHex(embMsgWest[i].AddrS()+ 0x10000l);	
					rBuffEmbMsgWest.Add(embMsgWest[i]);
#else  //ETH_TRANZIT
//printEthLongHex(embMsgWest[i].AddrS());//ok
...
//return;
//maked later ScanTxtFiles();// 1-3 sec
//ok return;
//unsigned char tmp;
time_start_prt = 0;
...
// return;
 //   mod_en = ModemIsUsed(0);
	...
	embSerialACT155.Init();	  //hier hangt!!!!
//ok2  return;
...
//test 111216 SetNeedInitAllProt(); //set flags to init protection that is saved in objekts
//later is used setneedallproti
...
//moved to operateleds2  outportb((BLOCK_SWEEP1 + MOD_LED_ADDR2),1);	//modem on
...
//	unStatePUM.statePUM.state[3] = inportb(0x523);
 ...
 //hier keept lock	unStatePUM.statePUM.state[5] = inportb(0x525);
...
//  printf("\n\r protectionUncorrectedBytes:%ld",   modemp[deviceindex].protectionUncorrectedBytes);
 ...
 unStatePUM.statePUM.state[2] |= CHANNEL1_LOCK_BIT;//2
 unStatePUM.statePUM.state[7] = 0;//2
 unStatePUM.statePUM.state[8] = 0;//2
...
//#endif
  ...
  {
   unStatePUM.statePUM.state[2] |= CHANNEL2_LOCK_BIT;//2
   unStatePUM.statePUM.state[9] = 0;//2
   unStatePUM.statePUM.state[10] = 0;//2
...
while(!(ModemIsUsed(deviceindex>>1)))  //hier cyckling
 {
  deviceindex++;
#ifndef PROG_BMDN4
  if((deviceindex>>1) == currentrezerv) deviceindex++;
  if((deviceindex>>1) == currentrezerv) deviceindex++; //in one modem two provingents
#endif
...
//___________________________________ add second
   /*
if(!( Ms.acquireStatus))
{
ans = PVG610_API_ModemStatusGet((deviceindex), 0, &Ms);
...
#else
//??????
//110606 removing to use requests of needupdate	 GetRxPhyIdD();	 //only for scan hier
#endif
...
//to hier
//110909 ScanModems();	 //find and load modems
