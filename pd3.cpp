...
union  uEmb5Prd400State
{
	 Emb5Prd400State state; 	 // Код канала РПУ (1..2)	
   //	unsigned char byte[11];
   //	unsigned char byte[12];
	unsigned char byte[PD3_SIZE];
};

uEmb5Prd400State pd3;


#include "pd3_eth_1.cpp"

extern "C" unsigned short  GetSerialPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0x98;
	    Buff[1] = 0;
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.serial = (unsigned char)*ans;
	   pd3.state.serial <<= 8;
	    Buff[0] = 0x96;
	    Buff[1] = 0;
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
   ...
   return pd3.state.freq;
}

extern "C" unsigned char  GetStatusPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0xA3;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.status = (unsigned char)*ans;
	  	
   return pd3.state.status;
}

extern "C" unsigned char  GetSoftPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0x9A;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.soft = (unsigned char)*ans;
	  	
   return pd3.state.soft;
}

extern "C" unsigned char  GetHardPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0x9E;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.hard = (unsigned char)*ans;
	  	
   return pd3.state.hard;
}

extern "C" unsigned char  GetTypePd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0xA1;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.type = (unsigned char)*ans;
	  	
   return pd3.state.type;
}

extern "C" unsigned char  GetTempPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0xA2;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.temp = (unsigned char)*ans;
	  	
   return pd3.state.temp;
}

extern "C" unsigned char  GetPowerPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0xA6;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.power = (unsigned char)*ans;
	  	
   return pd3.state.power;
}
...
#ifdef PD3_KEEP_PARAMS
  ReadDevId2(0);
  pd3.state.freq =	freqpd;
  pd3.state.att =	attpd;
...
extern "C" char  SetFreq(char *IPBuff)
{
  char Buff[2];
	   Buff[0] = 0x8;
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   SpiWriteCS2PD_Fast (1, (unsigned long)&IPBuff[1]);	
	   Buff[0] = 0x9;
	   SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[0]);	
	   SpiWriteCS2PD_Fast (1, (unsigned long)&IPBuff[2]);	
	   Buff[0] = 0xA;
       Buff[1] = 0;
	   SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[0]);	
	   SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	

	 #ifdef PD3_KEEP_PARAMS
	  	freqpd = (IPBuff[2] >> 8) + IPBuff[1];
        SetNeedWriteDevId2();
	 #endif
...
extern "C" unsigned short  GetFreqPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0x89;
 	   ...
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.freq = (unsigned char)*ans;
	   pd3.state.freq <<= 8;
	    Buff[0] = 0x88;
	    Buff[1] = 0;
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
...
extern "C" unsigned short  GetAttPd(void)
{
  char *ans;
  char Buff[2];
	   Buff[0] = 0x82;
	   Buff[1] = 0;
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
	   ans = SpiWriteCS2PD_Fast (1, (unsigned long)&Buff[1]);	
	   pd3.state.att = (unsigned char)*ans;


	 	
   return pd3.state.att;
}
...
extern "C" char      SetAtt(char *IPBuff)
{
char Buff[2];
	   Buff[0] = 0x2;
 	   SpiWriteCS2PD (1, (unsigned long)&Buff[0]);	
...
//printfp("\n\r11");
  ...
  for(long i = 0; i < PD3_SIZE; i++)
  {
	 add_char_TCP_buff(pd3.byte[i]);

 if(GetTestMode() == 0x50)
//if(1)
{
printfpd(" %02X ", pd3.byte[i]);
}
...
extern "C" void Parse_PD3(IPBuff,  len)
{
...
//if(1)
{
  ...
  switch(*IPBuff)
	{
		case 1:   ParseCommand1();  len-= 1;    IPBuff++;        break;
		case 2:	  ParseCommand2(IPBuff);  len-= 3;   IPBuff++;   IPBuff++;   IPBuff++;   break;
		case 3:	  ParseCommand3(IPBuff);  len-= 2;  IPBuff++;   IPBuff++;   break;
		case 4:	  ParseCommand4(IPBuff);  len-= 2;  IPBuff++;   IPBuff++;   break;
