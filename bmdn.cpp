...
public :
 ...
 //unsigned long CheckErrors(void);
 unsigned long CurrentErrors;
 unsigned char Init();
};

 unsigned char Modem::Init()
{

 return 0;
}

 unsigned long CheckErrors(unsigned char im)
{
//im - channel
// CurrentErrors = 0;
 return 0;
...
Modem Modem[MODEM_QUANTITY];


extern "C" void ModemStatusGetm()
{
...
extern "C" void ConfigFileRunm()
{
 for(unsigned long im = 0; im < MODEM_QUANTITY; im++)
 {
  printf("\n\r PVG610_API_ConfigFileRun  %d", im);
  PVG610_API_ConfigFileRun (im,0);
 }
}


#define CONFIG_FILE_VARIANTS_STEP (10000)
#define MAX_CFG_FILE_VARIANT (100)	//максимум	100 конфигураций

extern "C" void ConfigFileLoadm(unsigned long variant)
{
PVG610_MODEM_STATUS_STRUCT	Ms;
...
for(unsigned long im = 0; im < MODEM_QUANTITY; im++)
{
 printf("\n\r PVG610_API_ConfigFileFromHostLoad  %d", im);
 PVG610_API_ConfigFileFromHostLoad(0, 0, pBuffer, len);
}
}


extern "C" void FirmwareLoadm()
{
PVG610_MODEM_STATUS_STRUCT	Ms;
PVG610_MODEM_STATUS_STRUCT *pMs;
pMs = &Ms;
...
for(unsigned long im = 0; im < MODEM_QUANTITY; im++)
{
 printf("\n\r PVG610_API_FirmwareFromHostBoot  %d", im);
 PVG610_API_FirmwareFromHostBoot(im, pBuffer, len, PVG610_DEVICE_RESET_CPU_E);  
}
...
void StartModems()
{
 char ret;
FirmwareLoadm();
ConfigFileLoadm(GetNumConfigFile());
ConfigFileRunm();
ModemStatusGetm();  //old data
ModemStatusGetm();
//delay_mcs(10000l);
...
switch(staterezerv)
 ...
 case 0:   //rezerv do not used
			...
			if(max_errors > ERRORS_THRESHOLD)
			{
			   SetModemToRezerv(imax);	
...
//090802 unsigned long cnt_req, cnt_ans, cnt_esn, cnt_erc, cnt_loop;//, EthSendTimer; //, EthAnsTimer;
...
//t090708		AddCharEastE(eastByte); //t
 ...
 //  			 if(1)
   		   	{
   			 buff_cashe_modem.add(embMsgEast.body[1], embMsgEast.AddrS(), embMsgEast.AddrI(), embMsgEast.AddrR());
			 /*	 
 ...
 //090709	 tBuffEmbMsgEthAll1.Add(embMsgEast);	  //t0129
  ...
  //t0129 	 SendEastTransitToEth();
  	 buff_cashe.add(embMsgEast.body[1], embMsgEast.AddrS(), embMsgEast.AddrI(), embMsgEast.AddrR());
 //	  EthSendTimer =	(( ETH_SEND_TIMER) * ((BSP_ALARM_FREQUENCY * CORE_ALARM_FREQUENCY) / 1000l));
  // 	 return 1;
   }
...
for(unsigned long im= 0; im < MODEM_QUANTITY; im++)
{
PVG610_API_ComConfigure(im, PVG610_COM_MEDIA_SPI_E, 200000000l);
}
...
//have timeout PVG610_API_FlashFirmwareBurn(0, 1, pBuffer, len); // PVG610_FIRMWARE_INDEX_ENUM try 1
...
//PVG610_ERROR_MSG_ENUM PVG610_API_FirmwareFromHostReload(BYTE deviceIndex, BYTE *pBuffer, UINT32 len)
// PVG610_API_FirmwareFromHostReload(0, pBuffer, len);
//PVG610_ERROR_MSG_ENUM PVG610_API_FPI_FFirmwareBurn(BYTE deviceIndex, PVG610_FIRMWARE_INDEX_ENUM firmwareIndex, BYTE *pBuffer, UINT32 len)
//have timeout PVG610_API_FlashFirmwareBurn(0, 1, pBuffer, len); // PVG610_FIRMWARE_INDEX_ENUM try 1
// PVG610_API_FlashFirmwareBurn(0, 0, pBuffer, len); // PVG610_FIRMWARE_INDEX_ENUM try 0 
 //  printEthStringAdd(1,0x50);

//for(long i = 0;  i < PROVHEADER; i++)
//{
//  pBuffer++;
//}

//okfor(long i = 0;  i < 0x66; i++)
//okfor(long i = 0;  i < 0x65; i++)
//okfor(long i = 0;  i < 0x64; i++)
//bad!!!!for(long i = 0;  i < 0x63; i++)
//{
// pBuffer++;
//}
...
unsigned char TRBUFF[] = {0x18,1,2,3,0x19};
for(i = 0;  i < 5; i++)
{
  Transmit0c((unsigned char) TRBUFF[i]);

}

unsigned char TRBUFF1[] = {0x18,0,0,0,0x19};
for(i = 0;  i < 5; i++)
{
  Transmit0c((unsigned char) TRBUFF1[i]);
...
//____________________________________________
  // Transmit0c((unsigned char) (0x7E));
/*
unsigned long currword = 0;
unsigned long wcnt = 0;										  
unsigned long dcnt = 0;
 */
...
/*
 ...
 if(currword != dcnt)
 {
   printf("\n\r dcnt = %lX currword = %lX", dcnt, currword);
 }
  dcnt++;
  currword = 0;
...
//have timeout PVG610_API_FlashFirmwareBurn(0, 1, pBuffer, len); // PVG610_FIRMWARE_INDEX_ENUM try 1
// PVG610_API_FlashFirmwareBurn(0, 0, pBuffer, len); // PVG610_FIRMWARE_INDEX_ENUM try 0 
 //  printEthStringAdd(1,0x60);

// pvg610FwSendAndJump(PVG610_DATABASE_STRUCT *pDbRecord, BYTE *pBuffer)
 SendAndJump(pBuffer);
...
unsigned long dcnt = 0;
 */
//for(i = 0;  i < ProvSizeToDownloadD; i++)
//{
...
//t100422 printf(" %02X", * BufferDataTmp);
...
//write_xil_char(3, 0x2);
//write_xil_char(2, 0x85);
//write_xil_char(2, 0x81);
//write_xil_char(3, 0x1);
...
// 532-533	Счётчик числа принятых пакетов	16 бит
 // if(err_timer++ > 1000l)
 if((timer > error_timer) && ((timer > error_timer) > PERIOD_REQ_ERR))
   	 {
	 ...
	 switch(current_channel)
	 {
	  case 0:
				...
				unStatePUM.statePUM.ulErrBeforeRS1 				  =   Fc.rsErrorByteCounter;
	   		  ...
	   		  break;
	  case 1: 
		      unStateBMD155.stateBMD155.count_notRS2 				=	 Fc.fecUncorrectedBlockCounter;
 			  unStatePUM.statePUM.ulErrAfterRS2 				   +=	 Fc.fecUncorrectedBlockCounter;

			  unStateBMD155.stateBMD155.count_no_correct_block2 	=	 Fc.rsErrorByteCounter;
			  unStatePUM.statePUM.ulErrBeforeRS2 					=    Fc.rsErrorByteCounter;
...
//  unStateBMD155.stateBMD155.count_notRS2 =
...
current_channel++;
 if(current_channel > MODEM_QUANTITY)
 {
 current_channel = 0;
 }
	}
...
extern "C" unsigned char SetLoop(unsigned char loop,unsigned long milli, unsigned char nearend, unsigned char trib)
{
 ...
 lbs.nearEnd = nearend;
 lbs.tributary = trib; //0;	 //use modem loopback

 ret = PVG610_API_LoopbackSet(0,0, &lbs);
...
//extern "C" void LoadSyntm(unsigned char deviceindex)
...
//_________________________________________________________________
	d[0] = 0x20; d[1] = 0x15; d[2] = 0; d[3] = 0x3c; dlen = 4;
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0; d[1] = 0x16; d[2] = 0x01; dlen = 3;
	WriteSpiB(dlen, d);
//_________________________________________________________________
...
//_________________________________________________________________
	...
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0x41; d[1] = 0x92; d[2] = 0; d[3] = 0; d[4] = 0x11; dlen = 5;
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0x41; d[1] = 0x98; d[2] = 0; d[3] = 0; d[4] = 0x11; dlen = 5;
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0x01; d[1] = 0x99; d[2] = 0x11;  dlen = 3;
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0x01; d[1] = 0x9c; d[2] = 0x20;  dlen = 3;
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0x01; d[1] = 0x9e; d[2] = 0x11;  dlen = 3;
	WriteSpiB(dlen, d);
//_________________________________________________________________
	d[0] = 0x01; d[1] = 0xa1; d[2] = 0x20;  dlen = 3;
	WriteSpiB(dlen, d);
//_________________________________________________________________
