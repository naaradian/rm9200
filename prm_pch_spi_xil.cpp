...
void EmbMsgN::SetCommandN(unsigned long pos)
{
 switch(body[pos++])
 {
  ...
  default 	: Ready_Reseive = 1; break;	  //undefined command
 }


}

unsigned char EmbMsgN::ParseN(void)	  //used for usart
{
parsed_now = 1;
...
extern "C" void	ClearSpi(void)
{
	outportb(OPERATE_SPI_XIL,0xE0);		//reset rx and tx  and bit ordering
	outportb(OPERATE_SPI_XIL,0x20);	   //keep bit ordering = 1
}
...
#ifndef TEST_ADDR
MY_ADDRESS_XIL = inportb(ADDR_SPI_XIL);
#else
MY_ADDRESS_XIL = 0xF;
#endif
...
{
 outportb(ALARMOP_SPI_XIL,0);
}

extern "C" void SetLineAlarmToOneX()
{
  outportb(ALARMOP_SPI_XIL,1);
}
...
{
 outportb(DOP_SPI_XIL,0);
 outportb(DATA_READY_SPI_XIL,0);	 //tx disable

}

extern "C" void SetLineDataSPIToOneX()
{
  outportb(DATA_READY_SPI_XIL,1);	 //tx enable
  outportb(DOP_SPI_XIL,1);			//
}

extern "C" void SetLineDataSPIOffX()
{
 outportb(DATA_READY_SPI_XIL,0);
 outportb(DOP_SPI_XIL,1);
}
...
//if(*b & MASK_BIT_7) mask |= MASK_BIT_7;
 ...
 outportb(S_SPI_XIL, S_reg );
 outportb(N_SPI_XIL, N_reg );
...
//04piob_ptr->CODR = maskb;
outportb(ATT3_SPI_XIL,att3_reg);
...
extern "C" unsigned long ParseCommandSSC2( unsigned char *Ans)	  //used for spi
{
...
#ifndef POLLING_TRANSMIT

#ifdef  IncEmbSerialSPI
	  embserialspi.AddMod(embMsgSSC2N.GetAns(i));
#endif

#else  //polling_transmit

 outportb(DATA_SPI_XIL,embMsgSSC2N.GetAns(i));
...
//#ifndef TEST_ANS_USART	  //only foe check and operate in zal
//	   ToPdcTu0(embMsgSSC2N.GetAns(i));		 //???? this need ?
//#endif

   //    printfpd(" %02X",embMsgSSC2N.GetAns(i));	 //???? this need ? 
//#endif 
           }


//#ifdef  IncEmbSerialSPI
//  hier need waiting to data rewrited to xilinx tghrough interrupt!!!!

#ifndef POLLING_TRANSMIT
 delay_mcs(100l); //wait finished interrupts
#endif

SetLineDataSPIToOneX();
//hier need init  embMsgSSC2N
 embMsgSSC2N.Init();
...
unsigned char Data[] = {0xAA, 0xff, 1, 0, 1};	   //command 0
//wrong! unsigned char Data[] = {0xAA, 0x0f, 1, 0, 0x11};	   //command 0	  addr 0xf	-wrong!	  need + mask request
//unsigned char Data[] = {0xAA, 0x8f, 1, 0, 0x91};
