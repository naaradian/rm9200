...
//#endif
  ...
  uProt[i].protection.PhyIDRx  = mod << 1;
	...
	/*
  uProt[i].protection.PhyIDTx  =  1;
  uProt[i].protection.PhyIDRx  =  0;
	 */
...
//#endif
  ...
  uProt[i].protection.flagchrx = 1;
  uProt[i].protection.flagchrxf = 0;
  uProt[i].protection.PhyIDTx  = SelfVer1();// 9;//1; //mod << 1;
  uProt[i].protection.PhyIDRx  = 0;// mod << 1;
  uProt[i].protection.CopyAcm = 1; //t 110511
...
//  printf("\n\r PVG610_API_ProtectionTxModeSet %d %X %X %X", provnum,uProt[provnum].protection.ProtTx, uProt[provnum].protection.PhyIDTx + ID_CORR,  uProt[provnum].protection.CopyAcm);     
     ret =  PVG610_API_ProtectionTxModeSet (provnum,
									                ...
									                // ,
                                                    SelfVer1(),// uProt[provnum].protection.PhyIDTx,
                                       //           	  1
                                                  	  //,
...
//151028                                                      uProt[provnum].protection.PhyIDRx + ID_CORR);
                                                     0); //151028
	  }
   }
...
// delay_mcs(10000l);//delay 10 ms
 ...
 //  if(configfilefext1[variant].LengthTxt ==  TXT_CUR_REAL_LEN)
                //    {
...
if(!CheckEthMode(GetModEth() & 0xf))
{
 ...
 #ifdef PRINT_ETH
	printfp("\n\r Wrong Eth Mode! Is setted default!");
#endif

}

#ifdef PRINT_ETH
	printfpd("\n\r GetModEth1 %02X" , GetModEth() );  //11
#endif
...
//				  unsigned char reset,  unsigned char changephy );
 mode_eth = (unsigned char)GetModEth();

//#ifdef PRINT_ETH
//	printfpd("\n\r GetModEth2 %02X" , GetModEth() );  //11
//	printfpd("\n\r mode_eth 2 %02X" ,mode_eth );  //11
...
//unEmb2TypeVer.emb2TypeVer.signature_hardware[5] &= 0x1F;
if(mode_eth & ETH_10) {speed = 0; interf = 0; }
else if(mode_eth & ETH_100) {speed = 1; interf = 0;}
else if(mode_eth & ETH_1000) {speed = 2; interf = 1;}
else {speed = 2; interf = 1;}	   //default
...
//#ifdef PRINT_ETH
  //	printfpd("\n\r GetModEth3 %02X" , GetModEth() );  //11
//#endif


#ifdef PRINT_ETH
 printfpd("\n\r mode_eth 4: %02X", mode_eth);
#endif
...
extern "C" void StartEth_old(unsigned char flagm)
{
//unsigned long ret;
//unsigned long ir;
unsigned char auton;
...
#endif
//printfp("1210045");
ConfigDataLoadm(GetNumConfigFile(flagm-1),flagm);	//deleted error
...
ConfigFileLoadm(GetNumConfigFile(flagm-1), flagm);	 //deleted error
if(!hot_restart)
 {
ConfigFileRunm(flagm);
#ifdef USE_CALIBR
...
//#define TIME_WAITING_FAR_END	  (STATE_SCAN_PERIOD + 2000l)
...
// #ifdef  DEBUG_TRANZIT 
 //       if( embMsg232.AddrS() == 0x6431) {  printfp("_t_");}
    //    if( embMsgWest[i].AddrI() == 0x7766) {  printfpd("_mod west_ %d", cnt_mod);}
//#endif
...
//170117 not need ?   oldtime[i] =  time1;
   outportb((BLOCK_SWEEP1*(i + 1) + FAR_LED_PORT), req);
...
//		if(!recieveEast)
	   ...
	   //			printfpd("sch:%d", chn);
		  ...
		  //		 #ifdef DEBUG_TRANZIT
		   //			  if(i == 1) printfpd(" E : %02X ", embMsgA.body[i])	;
		 //		#endif
...
extern "C" void  CheckLockS(void)		//maked only for wariant - one provingent on plata
{
 ...
 //110909 this maked only at start	modemp[deviceindex].TimesTryGetLock = 0;
   ...
   {
	 if(time1 < modemp[deviceindex].LastTimeInLock)
	 {
	   modemp[deviceindex].LastTimeInLock = time1;
	 }
...
//static unsigned char prev_num;
//unsigned char pak_poz;
   ...
   //		embMsg485Request_1.SetReadyToSend();
	...
	else
	{
	   ...
	   else if((embMsgRequest->Body(1))== ETH_COMMAND_PORT)
		{
			...
			SetModForSS1((embMsgRequest->Body(0) & MASK_BIT_5) >> 7);
 		}
		  */
		else
		{
...
void EthStatusGetXil()	 //from sat modem 
{
// static unsigned char laststate;
 // static unsigned char OldStatus;
...
switch (Status & 0x1f)
{
  case 1: speed = 0;  duplex = 0; interf = 0; sign_hw5 |= ETH_10; break; //10h
  case 2: speed = 1;  duplex = 0; interf = 0; sign_hw5 |= ETH_100;  break; //100h
  case 4: speed = 2;  duplex = 0; interf = 1; sign_hw5 |= ETH_1000; break; //1000h
  case 9: speed = 0;  duplex = 1; interf = 0; sign_hw5 |= ETH_10;  sign_hw5 |= ETH_DUPL;  break; //10f	   0x09
  case 10: speed = 1; duplex = 1; interf = 0; sign_hw5 |= ETH_100;  sign_hw5 |= ETH_DUPL; break; //100f   0x0a	
  case 12: speed = 2; duplex = 1; interf = 1; sign_hw5 |= ETH_1000;  sign_hw5 |= ETH_DUPL;   break; //1000f  0x0c	
  case 20: speed = 2; duplex = 0; interf = 1; sign_hw5 |= ETH_1000;  sign_hw5 |=  ETH_OPT;  break; //1000lxh	 0x14
  case 28: speed = 2; duplex = 1; interf = 1;  sign_hw5 |= ETH_1000; sign_hw5 |= ETH_DUPL; sign_hw5 |=  ETH_OPT;  break; //1000lxf	
  default : speed = 9; duplex = 9; interf = 1; break;       //no link
}
...
//  unEmb2TypeVer.emb2TypeVer.signature_hardware[5] =	Status;
//  unEmb2TypeVer.emb2TypeVer.signature_hardware[5] |= (GetModEth() & ETH_AUTO) +((Status & 0x1f) << 1) + (GetModForSS1() << 7); //	  
  unEmb2TypeVer.emb2TypeVer.signature_hardware[5] = (GetModEth() & ETH_AUTO) + sign_hw5 + (GetModForSS1() << 7); //
#ifdef PRINT_ETH
printfpd("\n\rxil: %02X",Status);
printfpd(" sign_hw5: %02X",unEmb2TypeVer.emb2TypeVer.signature_hardware[5]);
printfpd(" speed: %d ",speed);
