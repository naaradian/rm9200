...
//#include "kbd.h" //TCPParser
//void TCPParser(unsigned char*);
char* TCPAnswer();
void GetFlash(unsigned char);
char GetState(void);
void ProgModeClose(void);
//void TCPSetRemBuf();
// constants
...
//	unsigned int cnt =;
	unsigned char* pBuff;
//	unsigned int TCPBytesToSend = 64;
...
//	} while(p);
	...
	{
/*t		
		if((((*pBuff) & 0xff) == ';') || (((*pBuff) & 0xff) == '.')
		||	(((*pBuff)>>8 & 0xff) == ';') || (((*pBuff) & 0xff)>>8 == '.')
...
*/
		...
		GetRem((*pBuff)>>8 & 0xff);
//		pBuff++;
//		if(SetNrOfDB >= 2) SetNrOfDB -= 2;
//		else SetNrOfDB--;
		}
		else
		{
//		GetFlash((*pBuff) & 0xff);
//		GetFlash((*pBuff)>>8 & 0xff);
...
//		SetNrOfDB --;
	...
	if(!GetState())
	{
	pAnswer = TCPAnswer(); //pTbuff for serial remote
    memcpy8(TCP_TX_BUF, pAnswer, 20);//answer 64 Byte
    TCPTxDataCount = 20;        // bytes to xfer
...
void DoNetworkStuff(void)
{
  ...
  unsigned char tmp, tmpint, tmp1, tmp2;
...
//2  Write8900(ADD_PORT, PP_RxEvent); 
                // point to RxEvent
 // MDelay(10000l); //to test
//  ActRxEvent = Read8900(ADD_PORT);
  
//2  ActRxEvent = Read8900(DATA_PORT);              // read, implied skip of last frame
//	ActRxEvent = CP2200regs.CPINFOL;
//  ActRxEvent = Read8900(DATA_PORT); 
//	MDelay(10000l); //to test
 //2 if (ActRxEvent & RX_OK)
 //tif ((CP2200regs.CPINFOL & RX_OK2))
// if (1)
//tmp1 = CP2200regs.INT0RD & 0xff;
 //if (CP2200regs.INT0RD & ERXINT)
 //if (tmp1 & RXINTR2)
 // CP2200regs.CPADDRH = 0;
 //  CP2200regs.CPADDRL = 0;
 //MDelay(500l);
 //tmp1 = CP2200regs.CPINFOH & 0xff;
 //MDelay(500l);
 // tmp2 = CP2200regs.CPINFOL & 0xff;
 // MDelay(500l);
// if(tmp1 & RXINTR2)
 //	if(1)
 //	if(!(CP2200regs.RXSTA & RXBUSY2))
 if(!(CP2200regs.RXSTA & RXBUSY2))
   //       && (!(CP2200regs.TXBUSY & TXBUSY2)))
   {
//   tmp1 = CP2200regs.INT0;
 // tmp1 = CP2200regs.INT1;
//   tmp1 = CP2200regs.INT0; //clear
 // _wait_ms(10);
 //MDelay(1000l);
  // if ((CP2200regs.CPINFOH & RXVALID2) || (CP2200regs.CPINFOL & RX_OK2))
 // if ((CP2200regs.CPINFOH & RXVALID2) && (CP2200regs.CPINFOL & RX_OK2))
  if ( (CP2200regs.CPINFOH & RXVALID2) ||(CP2200regs.CPINFOH & RXVALID2))
// if (CP2200regs.CPINFOL & RX_OK2)
  // if ((tmp1 & RXVALID2) || (tmp2 & RX_OK2))
//	if(1)
//if ((CP2200regs.CPINFOL & RX_OK2)&&(!(CP2200regs.RXSTA & RXBUSY2)))	
 // if(CP2200regs.CPINFOL & RX_OK2)
// if(1)  //if (CP2200regs.CPINFOH & RXVALID2)
  {
  // 	GpioDataRegs.GPBSET.all = 0x0080;
  // 	if(1) //test
  // 	{
 // MDelay(1000l);
  //  CP2200regs.CPADDRH = CP2200regs.RXFIFOHEADH;
 //  CP2200regs.CPADDRL = CP2200regs.RXFIFOHEADL;
 //2   if (ActRxEvent & RX_IA) ProcessEthIAFrame();//0x400
// GpioDataRegs.GPBCLEAR.all = 0x0080;
    if (!(CP2200regs.CPINFOH & BCAST2)) 
    {
 // CP2200regs.CPADDRH = CP2200regs.RXFIFOHEADH;
 // CP2200regs.CPADDRL = CP2200regs.RXFIFOHEADL;
    ProcessEthIAFrame();//0x400
  //   tmp = CP2200regs.CPTLB;
  //  CP2200regs.RXCN = RXCLEAR2;
  //  MDelay(100l);
  //  CP2200regs.RXCN = RXSKIP2; //skip current packet
  //  CP2200regs.RXCN = RXCLRV2;
 //  tmp = CP2200regs.CPTLB;
//  if(cnt == 7)
 //   {
   // CP2200regs.RXCN = RXCLEAR2;// cnt = 0;
 //   }
 //  }//no bcast
 //   else //cnt
 //   {
 //   cnt++;
 //   }
// while(CP2200regs.RXSTA & RXBUSY2)
 //CP2200regs.RXCN = RXSKIP2; //skip current packet
    }
//2    if (ActRxEvent & RX_BROADCAST) ProcessEthBroadcastFrame();//0x800
    else if((CP2200regs.CPINFOH & BCAST2) || (CP2200regs.CPINFOH & MCAST2))
    {
 // CP2200regs.CPADDRH = CP2200regs.RXFIFOHEADH;
//  CP2200regs.CPADDRL = CP2200regs.RXFIFOHEADL;   
//MDelay(1000l);
  			ProcessEthBroadcastFrame();//??? multicast where
   // tmp = CP2200regs.CPTLB;
  //   CP2200regs.RXCN = RXSKIP2;
   // CP2200regs.RXCN = RXCLRV2;
    // if (!(CP2200regs.RXSTA & RXBUSY2))
  //     CP2200regs.RXCN = RXCLRV2; 
//   tmp = CP2200regs.CPTLB;
...
// while(CP2200regs.RXSTA & RXBUSY2)
 ...
 // CP2200regs.RXCN = RXCLRV2;   CP2200regs.RXCN = RXCLRV2; 
 //if(cnt == 2)
   // {
   CP2200regs.RXCN = RXSKIP2;
    tmp1 = CP2200regs.INT0;
...
//  else
 // {
 // CP2200regs.RXCN = RXCLEAR2; //clear for test
  //MDelay(200l);
  // CP2200regs.RXCN = RXCLEAR2; //clear for test
 // MDelay(200l);
 //  CP2200regs.RXCN = RXCLEAR2; //clear for test
...
// while(CP2200regs.RXSTA & RXBUSY2)
 ...
 // }
  ...
  //  }
//   } //no rxok2
 // if (CP2200regs.INT0RD & RXFINTR) //hier clear interrupts
//if(!((CP2200regs.RXSTA & RXBUSY2) || (CP2200regs.TXBUSY & TXBUSY2)))
//{
  // tmp1 = CP2200regs.INT0; //clear interrupt
 //  CP2200regs.CPADDRH = CP2200regs.RXFIFOHEADH;
 //  CP2200regs.CPADDRL = CP2200regs.RXFIFOHEADL;
 //MDelay(1000l);
//    CP2200regs.RXCN = RXCLEAR2;
...
static void ProcessEthBroadcastFrame(void)
{
  ...
  //2ReadHB1ST8900(RX_FRAME_PORT);                   // ignore RxStatus Word
 // MDelay(100l);  
 // RecdFrameLength = ReadHB1ST8900(RX_FRAME_PORT);// get real length of frame 
  RecdFrameLength = ((unsigned int)CP2200regs.CPLENH << 8) + CP2200regs.CPLENL & 0xff;// get real length of frame 
 //  MDelay(100l); 
  DummyReadFrame8900(6);                         // ignore DA (FF-FF-FF-FF-FF-FF)
  CopyFromFrame8900(&RecdFrameMAC, 6);           // store SA (for our answer)
//	DummyReadFrame8900(2 ); //? length - cp2200
...
static void ProcessEthIAFrame(void)
{
  ...
  // next two words MUST be read with High-Byte 1st (CS8900 AN181 Page 2)
 //2 ReadHB1ST8900(RX_FRAME_PORT);                  // ignore RxStatus Word
 //2 RecdFrameLength = ReadHB1ST8900(RX_FRAME_PORT);// get real length of frame 
 RecdFrameLength = ((unsigned int)CP2200regs.CPLENH << 8) + CP2200regs.CPLENL & 0xff;// get real length of frame  
...
//            ProgModeClose();           
 ...
 // CP2200regs.RXCN = RXSKIP2; //skip current packet - it is not need
}
//------------------------------------------------------------------------------
// easyWEB internal function
