...
//?? need check crc?
  printfpd(" %02X",embMsgWest[i].Body(0));
  printfpd(" %02X",embMsgWest[i].Body(1));
 }
#endif
...
//#else // SEND_STATUS
//			  if((embMsgWest[i].AddrS()==SelfID()) || (embMsgWest[i].AddrS() == DEFAULT_ADDR))
//#endif // SEND_STATUS
#else  // ALL_ADDR	
				if(1)
#endif // ALL_ADDR	
...
// this is error - not testted	frorousedtransit : will brocken buffer!	embSerialACT155.GetTransit();
	...
	unEmb2TypeVer.emb2TypeVer.type       = 0x0110;
...
if(oldflag != flag)
{
 ...
 form_addr = BLOCK_SWEEP1 + MOD_LED_ADDR;
...
//need add alarm reg and lock req
  embSerialACT155.AddMod(0, chn);	//for test temporary
  embSerialACT155.AddMod(1, chn);
}
...
StatusMsg.SetBody(1,uerrsn15503work.errsn15303work.lock1);
