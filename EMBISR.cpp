...
{
  if(embRS232.UsedReceive())  return 1;
   return 0;
}
#else
extern "C" unsigned char haveusart1()
{
   ...
   if(embRS485.UsedReceive())  return 1;
   return 0;
}
#endif
...
void EmbRS232::TransE(unsigned char ch)
{
   ...
   {
 //t	cur_tbuffE = 2;
 // printfp("\n\r RESET TRANSMIT BUFFER \n\r");

	cur_tbuffE = 3;
   }
//  printfpd(" %d:", cur_tbuffE);
 //printfpd("%c", ch);
...
//150422error!!!!!!  tbuffE[0] = 	(char)cur_tbuffE >> 8;
  tbuffE[0] = 	(char)(cur_tbuffE >> 8);  //150422
  tbuffE[1] = 	(char)cur_tbuffE; // 
  tbuffE[2] = 	ETH_DIR;
...
//printfpd("\n\r cur_tbuffE : %d", cur_tbuffE);
//printfpd("\n\r cur_tbuffE : 0x%x", cur_tbuffE);
//printfpd("\n\r tbuffE[0] : 0x%x", tbuffE[0] );
//printfpd("\n\r tbuffE[1] : 0x%x", tbuffE[1] );

//printfp("\n\r___________________ TBuffe : \n\r");
//printfp(&tbuffE[15]);
//printfp("\n\r Finish __________ TBuffe : \n\r");
...
{
//port	asm{ sti }
//ok if(!(ttt%1000))printfp("INT");

	ttt++;
 if(embTimerCallback0) 
	{ 
 //if(!(ttt%1000))printfp("INT0");
...
#define KSS_TANZIT_STATUS3	(12)
#define KSS_TANZIT_STATUS4	(14)
...
unsigned char status_transit3;
unsigned char status_transit4;
...
//	status.byte = inportb(0x5A4);
	...
	status_transit1 = inportb(KSS_TANZIT_STATUS1);
 	status_transit2 = inportb(KSS_TANZIT_STATUS2);
 	status_transit3 = inportb(KSS_TANZIT_STATUS3);
 	status_transit4 = inportb(KSS_TANZIT_STATUS4);
...
//     while(irq.bit.IRQ_TX_trans && wd)
  ...
  //	 ttzc++;
	...
	if(irq.bit.IRQ_TX_UD1)
	   {
   wd = 15;
//     while(irq.bit.IRQ_TX_UD1 && wd)
	    while((status_transit3 & TX_READY_BIT) && wd && irq.bit.IRQ_TX_UD1)
   {
	 if(m_TBuffUD1.Used())
	  { 
	  outportb(0x50B,m_TBuffUD1.Get());
	  }
	  else {mask.bit.IRQ_TX_UD1=0;  break;}
...
if(irq.bit.IRQ_TX_UD2)
   {
   wd = 15;
   //  while(irq.bit.IRQ_TX_UD2 && wd)
    while((status_transit4 & TX_READY_BIT) && wd && irq.bit.IRQ_TX_UD2)
    {
	 if(m_TBuffUD2.Used())
	  { 
	  outportb(0x50D,m_TBuffUD2.Get());
	  	if(!m_TBuffUD2.Used())
    	   ...
    	   else {mask.bit.IRQ_TX_UD2=0;	 break;}
     irq.byte  = inportb(0x505);
	wd--;
	status_transit4 = inportb(KSS_TANZIT_STATUS4);
  ...
  //  printfpd("\n\rtwd :%d", wd );
	...
	if(irq.bit.IRQ_RX_UD1)
	{
	 ...
	 wd = 128;
	  ...
	  wd = 15;
#endif
	while(irq.bit.IRQ_RX_UD1 && wd)
	{
	 m_RBuffUD1.Add(inportb(0x50B)); 
     irq.byte  = inportb(0x505);
	 wd--;
	}
 //	wd--;
...
#ifdef PROG_COMMUTATOR1
#ifndef TEMPORARY_DO_NOT_STOP_RECEIVE
 	   mask.bit.IRQ_RX_UD1=0;	   //temporary commented
#endif
#endif

	}
#ifndef PROG_PU_M_V_A	
	if(irq.bit.IRQ_RX_UD2) 
	{
#ifdef PROG_COMMUTATOR1
	 wd = 128;
#else
	 ...
	 wd = 128;
	  ...
	  wd = 15;
#endif
	while(irq.bit.IRQ_RX_UD2 && wd) 
	{
	m_RBuffUD2.Add(inportb(0x50D)); 	   //090413
     irq.byte  = inportb(0x505);
	 wd--;
	}
 //	wd--;
...
#ifdef PROG_COMMUTATOR1
#ifndef TEMPORARY_DO_NOT_STOP_RECEIVE
 	   mask.bit.IRQ_RX_UD2=0;	   //temporary commented
#endif
#endif
...
//	UnEmbSerialMD34Status status;
	...
	unsigned char wd;
  		/*
     if(irq0_limit) irq0_limit--;
     if(!irq0_limit) {MaskIrq0();  }
  	   */
...
mask.byte = inportb(PROV_MASK_ADDR);
 	...
 	wd = 15;
 //	wd = 18;
  if(status_transit & TX_READY_BIT)
  {
  while((status_transit & TX_READY_BIT) && wd)
   {
 //  printfp("\n\r1");

 //  OperateBuffers_usart0t();

	if(m_TBuffTransit.Used())// &&  (irq0_limit))
	 { 
	 //  	 printfpd (".%02X.", m_TBuffTransit.Get());
	    txtrans++;
 ...
 //	outportb(PROV_MASK_ADDR,mask.byte);
 	...
 	status_transit = inportb(PROV_TANZIT_STATUS);
  //	 printfpd("\n\r st1: %02X", status_transit);
   //	  printfpd("mask: %02X", mask.byte);
...
// printfpd("\n\r_wd:  %ld", wd);
 ...
 //	status_transit = inportb(PROV_TANZIT_STATUS);  //for property working
  ...
  //	  irqm_byte = irqm.byte; 
   ...
   status_modem =  inportb((i+1)*STEP_ADDR + PROV_MODEM_STATUS);

			wd = 128;

            if(status_modem & RX_READY_BIT)
			{
  	 		while((status_modem & RX_READY_BIT) && wd)
//			if(irqm.bit.IRQ_RX_mod)
 //	   	   	while((irqm.bit.IRQ_RX_mod)	 && wd)
	 		{
...
//#endif
  ...
  //	 printEthStringAdd(1,byte);	   
   ...
   //	   	   printfpd("\n\rmr_wd:  %ld", wd);
#ifndef TEMPORARY_DO_NOT_STOP_RECEIVE
   	   maskm.bit.IRQ_RX_mod=0;	   //disable rx to next period	 writing mask in end
#endif
		   }
		...
		{
	    while((irqm.bit.IRQ_RX_UD1) && wd)
   //	 	while(irqm.bit.IRQ_RX_UD1)
			{
	...
	{
	while((irqm.bit.IRQ_RX_UD2)  && wd)
   //	while(irqm.bit.IRQ_RX_UD2)
			{
...
//#endif
				wd = 15;
	 //			if(1)
  	 		if(status_modem & TX_READY_BIT)
			{
  	 		while((status_modem & TX_READY_BIT) && wd)
	   //		while((status_modem & TX_READY_BIT) ) //t
   			{
  		  //	  printfp(" 4 ");
 //  OperateBuffers_usart0t();

	 	//	if((m_TBuffMod[i].Used()) && (irq0_limit))
   ...
   //	  printEthStringAdd(2,0);
    		...
    		outportb((i+1)*STEP_ADDR + PROV_DDC_DATA_ADDR_MOD,byte);
 			...
 			//	if(!m_TBuffMod[i].Used()){ maskm.bit.IRQ_TX_mod=0; 
				if(!m_TBuffMod[i].Used())// || (!(irq0_limit)))
				{ maskm.bit.IRQ_TX_mod=0; 
		   	  //	outportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD,maskm.byte);

 		   	 	break;
 				} //where it is init?????
  //				if((!m_TBuffMod[i].Used()) || (modem_bytes_counter > CHANNELS_RATE_LIMIT)) maskm.bit.IRQ_TX_mod=0;  //where it is init?????
   	  	  //	  wd--;
   	//			wd++;  //t
	  	 //	 status_modem =  inportb((i+1)*STEP_ADDR + PROV_MODEM_STATUS);
		 //	   printfpd("%X", status_modem );
...
//____________________________________________________________________________  
 ...
 outportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD,maskm.byte);
//!!!!!!!!!!!!!!!!!!!! hier need make off common modem interrupt!!!!!
 //t mask.bit.IRQ_mod = 0;		//121205
 }

 outportb(PROV_MASK_ADDR,mask.byte);
}

void EmbSerialMD34AVTProv::MaskReceive() 
{
unsigned long i = 0;
	  mask.byte = inportb(PROV_MASK_ADDR);
	  maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
      ...
      mask.bit.IRQ_RX_trans=0;
  //	  mask.bit.IRQ_mod=0;  //need keep updowm
      maskm.bit.IRQ_RX_mod=0;
	  maskm.bit.IRQ_RX_UD1=0;
	  maskm.bit.IRQ_RX_UD2=0;
// ok return;
//ok return;
...
void EmbSerialMD34AVTProv::UnMaskReceive() 
{
unsigned long i = 0;
	  mask.byte = inportb(PROV_MASK_ADDR);
	  maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
...
void EmbSerialMD34AVTProv::ChangeReceive1() 
{
unsigned long i = 0;
	  mask.byte = inportb(PROV_MASK_ADDR);
	  maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
...
void EmbSerialMD34AVTProv::ChangeReceive2() 
{
unsigned long i = 0;
	  mask.byte = inportb(PROV_MASK_ADDR);
	  maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
...
void EmbSerialMD34AVTProv::MaskAll() 
{
unsigned long i = 0;
...
void EmbSerialMD34AVTProv::UnMaskAll() 
{
unsigned long i = 0;
	  mask.byte = inportb(PROV_MASK_ADDR);
	  maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
...
void EmbSerialMD34AVTProv::AddUD1M(unsigned char byte, unsigned long i)
{
	 ...
	 */
void EmbSerialMD34AVTProv::SetMaskTransUD1(unsigned long i)
{
 //	m_TBuffUD1[i].Add(byte); 
	UnEmbSerialMD34IRQProvMod maskm;
	maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
...
{ 
  ...
  */
void EmbSerialMD34AVTProv::SetMaskTransUD2(unsigned long i)
{ 
//	m_TBuffUD2[i].Add(byte); 
	UnEmbSerialMD34IRQProvMod maskm;
 	maskm.byte = inportb((i+1)*STEP_ADDR + PROV_MASK_ADDR_MOD);
...
void FillEthTransit(unsigned char* pbuff, unsigned long len)
{
  while(len--)
  {
	trnzRS232.rcvbufE.Add(*pbuff++);
  }
}

void FillEthTransit_485(unsigned char* pbuff, unsigned long len)
{
  while(len--)
  {
  embRS485.rcvbufE.Add((unsigned char)*pbuff++);
...
{
  if(len_udpt >= (BUFF_SIZE_DATAG - 1)) {len_udpt = 0;}
  len_udpt++;
  return len_udpt - 1;
}
...
{
  if(len_udpt_485 >= (BUFF_SIZE_DATAG - 1)) {len_udpt_485 = 0;}
  len_udpt_485++;
  return len_udpt_485 - 1;
}
    ...
    //     fputc(*pTBuff0, fusart0_ptr); 
		...
		//	 printfp(pTBuff3);
		 ...
		 //	dirRS232.Trans_RDN(COMP_DEF_MAC_END);
		   ...
		   dirRS232.Trans_RDN_Full(dirRS232.HostMAC);
