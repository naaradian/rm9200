...
pData = DataDDS;
*pData++ = 0;   *pData++ = 0;	 *pData = 0x05;
SendToDDS(LEN_DATA_DDS,  (unsigned long)&DataDDS[0]);

pData = DataDDS;
*pData++ = 0;   *pData++ = 0x12;	 *pData = 1;
SendToDDS(LEN_DATA_DDS,  (unsigned long)&DataDDS[0]);


pData = DataDDS;
*pData++ = 0;   *pData++ = 0x10;	 *pData = 2;
SendToDDS(LEN_DATA_DDS,  (unsigned long)&DataDDS[0]);
...
extern "C" unsigned char ReadDDS(unsigned long addr)
{
unsigned char * pData;
static unsigned char DataDDS[LEN_DATA_DDS];//
unsigned char tmp;
//tmp =  SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]);
// for(int i = 0; i < 1000l; i++)
// {
  pData = DataDDS;
 *pData++ = 0x80 + (0x1f & (addr>>8));   *pData = (addr & 0xff);	pData++; *pData = 0xff; // *pData++ = 0;
 tmp =  SpiReadN2 (LEN_DATA_DDS, (unsigned long)&DataDDS[0]);	 //+1: clocks for readed byte
 printfpd("\n\r %x : ",addr);
 printfpd(" %x ",tmp);
 //wd_reset();
// }
...
//tmp =  SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]);
 ...
 {
 pData = DataDDS;
 *pData++ = 0x80;   *pData++ = i;	 *pData = 0xff;//  *pData++ = 0x88;
 tmp =  SpiReadN2 (LEN_DATA_DDS, (unsigned long)&DataDDS[0]);	 //+1: clocks for readed byte
// printfpd(" %x : ",i);
 printfpd(" %x ",tmp);
 wd_reset();
...
//tmp =  SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]);
 ...
 {
 pData = DataDDS;
 *pData++ = 0x80;   *pData++ = 4;	 *pData = 0x88;//  *pData++ = 0x88;
 tmp =  SpiReadN2 (LEN_DATA_DDS, (unsigned long)&DataDDS[0]);	 //+1: clocks for readed byte
// printfpd(" %x : ",i);
 printfpd(" %x ",tmp);
 wd_reset();
...
pData = DataDDS;
//140606 *pData++ = 0;   *pData++ = 0;	 *pData = 0x88;
*pData++ = 0;   *pData++ = 0;	 *pData = 0x09;	  //make sdo
SendToDDS(LEN_DATA_DDS,  (unsigned long)&DataDDS[0]);

//ReadDDS(0);
//_____________________________________________________________try read
//pData = DataDDS;
//*pData++ = 0x80;   *pData++ = 0;	 *pData = 0x88;
//printfpd("\n\rread 0: %x ",SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]));
...
//_____________________________________________________________try read
//pData = DataDDS;
//*pData++ = 0x80;   *pData++ = 1;	 *pData = 0x0;
//printfpd("\n\rread1 : %x ",SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]));
...
//_____________________________________________________________try read
//pData = DataDDS;
//*pData++ = 0x80;   *pData++ = 0x20;	 *pData = 0x99;
//printfpd("\n\rread20 : %x ",SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]));
...
int short EmbMsg::IsEnd()
{
	if((body[0]) && (counter>10) && (counter==Length()+13))
				return 1;
	return 0;
}
...
extern "C" void SetFiltersN(void);
extern "C" unsigned short GetSerialN(void);
extern "C" unsigned short GetFreqInt(void);
extern "C" unsigned short GetVerN(void);
...
//AddToAns(0);   //140311
AddToAns(HARD_V); //150701
//curcrc += inportb(VER_XIL);
//curcrc += 0;	//140311
curcrc += HARD_V;	//150701
...
unsigned short prm_serial;
unsigned short prm_ver;
unsigned short prm_freq;





void EmbMsgN::RunCommandNA(void)
{
// if(GetTestMode() == 0x50)
//{
//printf("\n\rRunCommandN1");
//}
unsigned char curcrc = 0; 
unsigned short id =  GetDevId();
//unsigned short id =  prm_serial;
//AddToAns(MY_ADDRESS);
AddToAnsDirect(START_ANS);
AddToAns(GetAddrN());
//curcrc += MY_ADDRESS;
...
void EmbMsgN::RunCommandNC(void)
{
// if(GetTestMode() == 0x50)
//{
//printf("\n\rRunCommandN1");
...
AddToAns(COMMANDNC);
curcrc += COMMANDNC;
AddToAns((unsigned char)emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ);
curcrc += (unsigned char)emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ & 0xff;
AddToAns((unsigned char)(emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ >> 8));
curcrc += (unsigned char)(emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ >> 8);
AddToAns(0);  //att1 olwais zero
curcrc += 0;
AddToAns(Filtr);  //filtr
curcrc += Filtr;
AddToAns(prm_inp);  //input
curcrc += prm_inp;
...
void EmbMsgN::RunCommandND(void)
{
// if(GetTestMode() == 0x50)
//{
//printf("\n\rRunCommandN1");
...
AddToAns(COMMANDND);
curcrc += COMMANDND;
AddToAns((unsigned char)prm_ver);
curcrc += (unsigned char)prm_ver;
AddToAns((unsigned char)(prm_ver >> 8));
curcrc += (unsigned char)(prm_ver >> 8);
AddToAns((unsigned char)prm_serial);
curcrc += (unsigned char)prm_serial;
AddToAns((unsigned char)(prm_serial >> 8));
curcrc += (unsigned char)(prm_serial >> 8);
AddToAns(curcrc+1);
}
...
void EmbMsgN::RunCommandNB(unsigned char inp)
{
static long cnt = 0l;
unsigned char flag = 0;
...
extern "C" unsigned char CheckAtt(unsigned long att)
{
 //  printfpd("\n\r %d", freq);
 //  printfpd("min  %d", (unsigned long)MIN_FREQ);
 //  printfpd(" max  %d", (unsigned long)MAX_FREQ);
  	 ...
  	 if((att >= (unsigned long)MIN_ATT) && (att <= (unsigned long)MAX_ATT))
	 {
 //	 printfp("\n\r ok");
  				 return 1;
	 }
...
extern "C" unsigned char GetNeedWriteDevId2(void)
{
  ...
  if(DelayWriteTime2 > time1)
  {
     ...
     DelayWriteTime2 = time1 + DELAY_TIME_TO_WRITE;
  }
...
unsigned long time_dis_eth;
unsigned char flag_dis_eth;
#define LONG_ETH_TIME	(3)


#endif

unsigned char Ready_Reseive;
//unsigned short prm_serial;
...
//160729 to work usart1  Init_IOs_noxil();
...
//____________________________________________
 ...
 SetUsart1BaudRate(115200l);  //something can be setted after this only
...
ReadDevId2(0);
 ...
 if(!CheckAtt(emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2))
 {
   emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2 = MIN_ATT;
 }
...
// SetInputN();
 ...
 prm_serial = GetSerialN();
  prm_ver = GetVerN();
  prm_freq = GetFreqInt();


//#ifdef MAKET_PLATA
//printfpd("\n\r prm_serial : %d ", prm_serial);
//#endif

//#ifdef ETH_DEBUG_ENABLED
// printEthLongHex(prm_ver);
// printEthLongHex(prm_serial);
...
if(((time1 < time_dis_eth) ||  (time1 > (time_dis_eth + LONG_ETH_TIME))) && flag_dis_eth)
{
flag_dis_eth = 0;
my_int_enable_eth();	//t
 // printfpd("\n\re:%d", time1);
...
if(GetTestMode() == MODE_CHECK_PINS)
{
  Check_IOs_noxil();
}
...
//  if((standard_protocol_timer > TIME_FOR_STANDART_PROTOKOL) && (use_standard_protocol == 1))
  if((standard_protocol_timer > 1) && (use_standard_protocol == 1))
  {
//   printf("\n\r ...timer is ready");
   use_standard_protocol = 0;
...
extern "C" void RestoreAtt2(unsigned char num, unsigned long val)
{
switch(num)
{
case 1:     emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2 =  val;
   	   //		emb5StatusReciever1.emb5statusreciever.stepAtt =  emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt;   
 	   		break;
case 2:	    emb5CommandRecieverSetAtt2.emb5commandrecieversetatt.stepAtt2 =  val;
   	 //		emb5StatusReciever2.emb5statusreciever.stepAtt =  emb5CommandRecieverSetAtt2.emb5commandrecieversetatt.stepAtt;   
 	   		break;
case 3:	    emb5CommandRecieverSetAtt3.emb5commandrecieversetatt.stepAtt2 =  val;
   	 //		emb5StatusReciever3.emb5statusreciever.stepAtt =  emb5CommandRecieverSetAtt3.emb5commandrecieversetatt.stepAtt;   
 	   		break;
case 4:     emb5CommandRecieverSetAtt4.emb5commandrecieversetatt.stepAtt2 =  val;
   	   //		emb5StatusReciever1.emb5statusreciever.stepAtt =  emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt;   
 	   		break;
case 5:	    emb5CommandRecieverSetAtt5.emb5commandrecieversetatt.stepAtt2 =  val;
   	 //		emb5StatusReciever2.emb5statusreciever.stepAtt =  emb5CommandRecieverSetAtt2.emb5commandrecieversetatt.stepAtt;   
 	   		break;
case 6:	    emb5CommandRecieverSetAtt6.emb5commandrecieversetatt.stepAtt2 =  val;
   	 //		emb5StatusReciever3.emb5statusreciever.stepAtt =  emb5CommandRecieverSetAtt3.emb5commandrecieversetatt.stepAtt;   
 	   		break;
...
switch(num)
{
case 1:    return emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2;
case 2:    return emb5CommandRecieverSetAtt2.emb5commandrecieversetatt.stepAtt2;
case 3:    return emb5CommandRecieverSetAtt3.emb5commandrecieversetatt.stepAtt2;
case 4:    return emb5CommandRecieverSetAtt4.emb5commandrecieversetatt.stepAtt2;
case 5:    return emb5CommandRecieverSetAtt5.emb5commandrecieversetatt.stepAtt2;
case 6:    return emb5CommandRecieverSetAtt6.emb5commandrecieversetatt.stepAtt2;
...
//reg = ~(reg); //140428
 //_at91rm9200_pio_set(AT91RM9200_PIO_PORT_A, 25, AT91RM9200_PIO_PERIPHERAL_IO, 1);  //tmp
...
unsigned short ret;
unsigned long wd_wait = WAIT_TIMEOUT;
unsigned char  tmp;
//need clear input buffer


 OperateBuffers_usart1(); 	//clear input buffer

 while(have_data_usart1())
 {
  tmp = get_data_usart1();
//    #ifdef MAKET_PLATA
//printfp("\rclean buffer");
//#endif
...
if(have_data_usart1())
 {
  tmp = get_data_usart1();  //0xAA
//  #ifdef MAKET_PLATA
//printfpd("\r wd_wait : %d ", wd_wait);
//#endif
//#ifdef ETH_DEBUG_ENABLED
...
//}
 ...
 if(have_data_usart1())
 {
  ret = (unsigned short)get_data_usart1();
//  #ifdef MAKET_PLATA
//printfpd("\r wd_wait : %d ", wd_wait);
//#endif
//#ifdef ETH_DEBUG_ENABLED
...
ToPdcTu1((unsigned char)emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ);
OperateBuffers_usart1t();
...
ToPdcTu1((unsigned char)(emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ>>8));
//printfpd("\n\r SetFreqN  0C : %02X", (unsigned char)(emb5CommandRecieverSetFrequ1.emb5commandrecieversetfrequ.frequ>>8));
...
#ifdef MAKET_PLATA
 printfpd("\n\ratt : %d", emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2);
#endif
...
ToPdcTu1(0xAA);
ToPdcTu1(0x03);
ToPdcTu1(emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2);
OperateBuffers_usart1t();

//printfpd("\n\r att2: %x", emb5CommandRecieverSetAtt1.emb5commandrecieversetatt.stepAtt2);
...
//printfpd("\n\reth command : %d", current_command_type);
...
#endif
  case 0xA	: embMsgSSC2N.RunCommandNA(); break;
...
SetLineDataSSC2ToSSC2Data();	 //answer is ready
  ...
  Ready_Reseive  = 1;  //new receive is enabled
