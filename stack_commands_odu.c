...
///////////////////////////////////////	UPP /////////////////////////////////////////////
      ...
      //    printfpd("\n\r 0x%02X ", embMsgRequest->Body(0));
       //   printfpd(" 0x%02X ", embMsgRequest->Body(1));
       //   printfpd(" 0x%02X ", embMsgRequest->Body(2));
      //    printfpd(" 0x%02X ", embMsgRequest->Body(3));
   ...
   if(GetTestMode() == 0x50)
	 		{
             printEthLongHex(0x0A0A0A);
        //  	printEthLongHex(0x09 + (embMsgRequest->Body(1) << 8));
        /*
	  printEthStringAdd(0,0);
     
  //	  printEthStringAdd(1,embMsg485.Body(i));
      printEthLongHex(0x0A + (embMsgRequest->Body(1) << 8));
	printEthLongHex(0x0A + (embMsgRequest->Body(2) << 8));
...
//                     if(1)
  ...
  //	  printEthStringAdd(1,embMsg485.Body(i));
      printEthLongHex(0x09 + (embMsgRequest->Body(1) << 8));
	printEthLongHex(0x09 + (embMsgRequest->Body(2) << 8));
...
//________________________________________________________________________bmdn1
           ...
           //this is not correct   if(modemp[0].configfilefext.LengthTxt !=  TXT_CUR_REAL_LEN)
                  ...
                  {
                //t 160617   if(modemp[0].configfilefext1[embMsgRequest->Body(1)].LengthTxt !=  TXT_CUR_REAL_LEN)
                //     if(0)
                    if(!modemp[0].configfilefext1[embMsgRequest->Body(1)].LengthTxt) // test  !=  TXT_CUR_REAL_LEN) 
                    { 
        #ifdef FIND_CONF_ERR 
                   printfpd ("\n\r LengthTxt  %d !=  TXT_CUR_REAL_LEN", modemp[0].configfilefext1[embMsgRequest->Body(1)].LengthTxt );
        #endif            
...
//                     for(i=0; i < modemp[0].configfilefext.LengthTxt; i++)
...
#ifdef FIND_CONF_ERR                      
                       printfpd(" %02X", modemp[0].configfilefext2[embMsgRequest->Body(1)].ConfigTxtData.Data[i]);
#endif
                       embMsgAns.SetBody((i+1), modemp[0].configfilefext2[embMsgRequest->Body(1)].ConfigTxtData.Data[i]);
  ...
  UsedExp.State.Reg2.bit.WriteId = 1;
         ...
         case 0x57: //eth 
                  ...
                  MakeMEthMode((unsigned long) embMsgRequest->Body(0));  //convert to menu parameter 0...5
                   StartEth(1); //151005
             ...
             case 0x58: //eth 
                  printfpd("\n\r58 : %X",embMsgRequest->Body(0));
              //    SetModEth((unsigned long)embMsgRequest->Body(0));
              //     StartEth(1); //151005
                  ten_mhz =  embMsgRequest->Body(0);
                  unEmb2TypeVer.emb2TypeVer.signature_hardware[6] = ten_mhz;
						embMsgAns.SetType(0x58);
              //    embMsgAns.SetLength(8);
                  embMsgAns.SetLength(1);
                  embMsgAns.SetBody(0, ten_mhz);
  UsedExp.State.Reg2.bit.WriteId = 1;
                  UsedExp.State.Reg2.bit.Some = 1;
...
//							if(embMsgRequest->Body(0))// set
			  //				if(embMsgRequest->Body(0) == 1)// set
						 //	{
							   SetIPRemUDP(
						  //	   (embMsgRequest->Body(0) << 24) +
							//   (embMsgRequest->Body(1) << 16) +
							   (embMsgRequest->Body(0) << 8) +
							   	embMsgRequest->Body(1)
				 			   );
					//		}
					  //	    else  if (embMsgRequest->Body(0) == 0)
                  //    	{ 
                     embMsgAns.SetLength(2);	
						//	embMsgAns.SetBody(0,embMsgRequest->Body(0)); 	
						 //	embMsgAns.SetBody(0, (GetIPRemUDP() >> 24) & 0xff);
						//	embMsgAns.SetBody(1, (GetIPRemUDP() >> 16) & 0xff);
							embMsgAns.SetBody(0, (GetIPRemUDP() >> 8) & 0xff);
							embMsgAns.SetBody(1, GetIPRemUDP() & 0xff);
                  //   }
...
//							if(embMsgRequest->Body(0))// set
			  //				if(embMsgRequest->Body(0) == 1)// set
						 ...
						 //	   SetIPRemUDP(
						  //	   (embMsgRequest->Body(0) << 24) +
						  //	   (embMsgRequest->Body(1) << 16) +
						  //	   (embMsgRequest->Body(2) << 8) +
						  //	   	embMsgRequest->Body(3)
				 		  //	   );
					//		}
					  //	    else  if (embMsgRequest->Body(0) == 0)
                  //    	{ 
