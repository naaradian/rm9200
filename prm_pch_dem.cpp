...
//ok  return;
 ...
 m_load_xilinx5(3,0); //load xil 3,4 to file 0
...
{
		unsigned long tmp = (IPBuff[4] << 24) + (IPBuff[3] << 16) +  (IPBuff[2] << 8)  + IPBuff[1];
		if(CheckSymbolRate(tmp))
		{
		   SetSymbolRate(tmp);
		   LoadSymbolRate(tmp);
 		   SetNeedWriteDevId();
		   unEmb2TypeVer.emb2TypeVer.signature_software[0] =  IPBuff[1];
		   unEmb2TypeVer.emb2TypeVer.signature_software[1] =  IPBuff[2];
		   unEmb2TypeVer.emb2TypeVer.signature_software[2] =  IPBuff[3];
		   unEmb2TypeVer.emb2TypeVer.signature_software[3] =  IPBuff[4];
...
extern "C" void  ParseCommand3(char *IPBuff)
{
		...
		if(CheckFreqPCh(tmp))
		{
		   ...
		   LoadModulSK(); //it depended from freq
 		   SetNeedWriteDevId();
		   unEmb2TypeVer.emb2TypeVer.signature_software[4] =  IPBuff[1];
		   unEmb2TypeVer.emb2TypeVer.signature_software[5] =  IPBuff[2];
		   unEmb2TypeVer.emb2TypeVer.signature_software[6] =  IPBuff[3];
		   unEmb2TypeVer.emb2TypeVer.signature_software[7] =  IPBuff[4];
		   add_char_TCP_buff(TYPE_RUN_OK);
		   add_char_TCP_buff(IPBuff[0]);
		 }
		 else
...
extern "C" void  ParseCommand4(char *IPBuff)
{
		...
		if(CheckModulSK(tmp))
		{
		   ...
		   LoadModulSK();
 		   SetNeedWriteDevId();
		   unEmb2TypeVer.emb2TypeVer.signature_software[8] =  IPBuff[1];
 		   add_char_TCP_buff(TYPE_RUN_OK);
		   add_char_TCP_buff(IPBuff[0]);
		 }
		 else
