...
{
  static unsigned char used_channel;
  used_channel++;
...
//t090708			AddCharWestE(westByte);
			...
			case 0xA5: modeA5West[i]=1; markerWest[i]  = 1; ready_mod[i]=1; time3BlankMod[i] = /*time3Marker[i] =*/ time1; break; // Маркеров здесь НЕТ!
			case 0x5A: mode5AWest[i]=1; ready_mod[i]=1; break;
			default:
				if(markerWest[i]==1)	markerWest[i]=2; 
				if(recieveWest[i]==1)
				{
				 if(modeA5West[i]) { embMsgWestR.Add(westByte); modeA5West[i]=0;	}
...
//___________>
			...
			case 0xA5: markerWest[i]  = 1; ready_mod[i]=1; time3BlankMod/* = time3Marker[i] */= time1; break; // Маркеров здесь НЕТ!
			case 0x5A: mode5AWest[i]=1; ready_mod[i]=1; break;
			default:
				if(markerWest[i]==1)	markerWest[i]=2; 
				if(recieveWest[i]==1)
...
//___________________________________________________________
  ...
  modemp[deviceindex1].protectionUncorrectedBytes =   Fc.protectionUncorrectedctionRxBytesFromProtection;
  modemp[deviceindex1].protectionUncorrectedBytes =   Fc.protectionUncorrecteder;

  printf("mod :  %d  Status : %d errors:%ld", (deviceindex1), modemp[deviceindex1].acquireStatus, modemp[deviceindex1].protectionRxBytesFromProtection);
  }
  else
  {
...
while(mask)
 {
  ...
  if(inpByte & mask)
  {
   ...
   mask VG610_API_FlashFirmwareReload(BYTE deviceIndex, PVG610_FIRMWARE_INDEX_ENUM firmwareIndex);
...
*END*-------------------------------------------------------ed to port embisr to be called from timer isr
*END*-------------------------------------------------------ll EmbTimer::IsrXX for any exemplar, declared as type EmbTimer
    embTimer.IsrXX();
	TimerCounter ++;
   	TimerCounter1 ++;
#ifdef ETH_TRANZIT
...
//while(1)
 ...
 unsigned char  rm[MODEM_QUANTITY], rmo[MODEM_QUANTITY], rtz,rtzo, tm[MODEM_QUANTITY], ttz, tup0, tup1;     
  ...
  unsigned long rmc[MODEM_QUANTITY], tmc[MODEM_QUANTITY]1erc;
  unsigned char rup0, rup0o,  rup1, rup1o;
  unsigned long rmc[MODEM_QUANTITY], tmc[MODEM_QUANTITY]=  (unsigned int)INT0_FLAG;
   //t      while (IntFlag & tmp)
   //     if(IntFlag & tmp)
   //	 if(1)
	 //	{
...
//	if (0x80 & inportb(0xff2e)) // при появлении прерывания INT3
  ...
  //	bstate =  inportb(0x505);
		...
		ortb(0x505) &  0x01) != 0) && cnt)  // RX MODEM
				{
				 	rm=inportb(0x507);
					rmc++;
...
extern  "C" unsignedesSNMP;
}
extern  "C" unsigned long ttReset(void)
{
	return (timeres / 1000l);
}
extern  "C" unsignedMP = 0l;
   timeres = 0l;
   start_count = 0l;
}
...
switch(deviceindex)
{
 ...
 casype =  form_addr  + SPI_ADR; addr_send =  form_addr  + SPI_SEND_ADDR;type_channel = CHANNEL_A; break;
 casr  + SPI_ADR; addr_send = form_addr  + SPI_SEND_ADDR;type_channel = CHANNEL_B; break;
...
switch(deviceindex)
{
 ...
 case 3: addr =  BLOCK_SWEEP + (((deviceindex>>1)+1)<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_A; break;
 case 3: addr>>1)+1)<<4) +  SPI_PORT_ADDR; addr_type =  BLOCK_SWEEP + (((deviceindex>>1)+1)<<4) + SPI_ADR; addr_send =  BLOCK_SWEEP + (((deviceindex>>1)+1)<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_B; break;
...
switch(deviceindex)
{
 ...
 case 4: addr_status = 	BLOCK_SWEEP + (((deviceindex>>1))<<4) + ADDR_STATUS1; addr_data = BLOCK_SWEEP + (((deviceindex>>1))<<4) +  SLOCK_SWEEP + (((deviceindex>>1))<<4) + ADDR_STATUS1; addr_data = BLOCK_SWEEP + (((deviceindex>>1))<<4) +  Seviceindex>>1))<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_A; break;
...
switch(deviceindex)
{
 ...
 case 1: addr_status = 	BLOCK_SWEEP + (((deviceindex>>1))<<4) + ADDR_STATUS1; addr_data = BLOCK_SWEEP + (((deviceindex>>1))<<4) +  SPI_PORT_ADDR; addr_type =  BLOCK_SWEEP + (((deviceindex>>1))<<4) + SPI_ADR; addr_send =  BLOCK_SWEEP + (((deviceindex>>1))<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_B; break;
 case 2: addr_status = 	BLOCK_SWEEP + (((deviceindex>>1))<<4) + ADDR_STATUS1; addr_data = BLOCK_SWEEP + (((deviceindex>>1))<<4) +  SPI_PORT_ADDR; addr_type =  BLOCK_SWEEP + (((deviceindex>>1))<<4) + SPI_ADR; addr_send =  BLOCK_SWEEP + (((deviceindex>>1))<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_A; break;
 case 3: addr_status = 	BLWEEP + (((deviceindex>>1))<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_A; break;
 case 3: addr_status = 	BLI_PORT_ADDR; addr_type =  BLOCK_SWEEP + (((deviceindex>>1))<<4) + SPI_ADR; addr_send =  BLOCK_SWEEP + (((deviceindex>>1))<<4) + SPI_SEND_ADDR;type_channel = CHANNEL_B; break;
...
//printEthStringAdd(1,0x77);
//printEthStringAdd(2,0);
//ong milli, unsigned char nearend, unsigned char trib)
{
