...
*/
  ...
  sec = SecondsSinceEpoch + 11644473600ULL;
    ...
    printfpd(" sec0 : %d ",sec/1000000ULL);
       printfpd(" _ %d ",sec%1000000ULL);

  
//sec = SecondsSinceEpoch + (uint64)473600 + (uint64)11644 * (uint64)1000000;
 ...
 //  printfpd(" quadr : %d ",quadricentennials);   //0

  sec %= 12622780800ULL;
  //  printfpd(" sec_ : %d ",sec/1000000ULL);
  //     printfpd(" _ %d ",sec%1000000ULL);
  ...
  printfpd(" cent : %d ",centennials);   //0

  if (centennials > 3)
  {
    centennials = 3;
  }
  sec -= (uint64)centennials * 3155673600ULL;
   printfpd("\n\r sec1 : %d ",sec/1000000ULL);
       printfpd(" _ %d ",sec%1000000ULL);


  // Remove multiples of 4 years (incl. 1 leap day), can't be more than 24
  // (because multiples of 25*4=100 years (incl. leap days) have been removed)
  quadrennials = (uint)(sec / 126230400ULL); // 4*(365+1/4)*24*3600
  if (quadrennials > 24)
  {
    quadrennials = 24;
  }
  
    printfpd(" quadrennials : %d ", quadrennials);   //0
  ...
  sec -= annuals * 31536000ULL;
   ...
   printfpd("annuals : %d ",annuals);   //0

  
    printfpd(" sec3 : %d ",sec);    // == itow!!!! ok!!!!
