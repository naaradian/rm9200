...
extern "C" void	  SetPar051(float val)		  //rx freq
{
...
float tmpval =  MTEbN0.Value;



MTEbN0.Value = val;
MTEbN0.ChangingValue = val;

if(!CheckWEBData())
{
MTEbN0.Value = tmpval;
MTEbN0.ChangingValue = tmpval;
...
extern "C" void	  SetPar052(float val)		  //rx freq
{
...
float tmpval =  MTBER.Value;



MTBER.Value = val;
MTBER.ChangingValue = val;

if(!CheckWEBData())
{
MTBER.Value = tmpval;
MTBER.ChangingValue = tmpval;
...
extern "C" void	  SetPar053(float val)		  //rx freq
{
...
float tmpval =  MTBERE.Value;



MTBERE.Value = val;
MTBERE.ChangingValue = val;

if(!CheckWEBData())
{
MTBERE.Value = tmpval;
MTBERE.ChangingValue = tmpval;
...
//extern "C" void	  SetPar06B(unsigned char val)		  //config file 1 number
extern "C" void	  SetPar06B(unsigned long val)		  //config file 1 number
{
...
//MUnModul.ChangingValue = val;
   ...
   tmpval = ConfigFile1.Value;
	  ...
	  ConfigFile1.Value = val; //ReallyNumCfg[NamesConfigFiles.ChangingValue];
...
case 0x51:    SetPar051(par); break;	//treshold
case 0x52:    SetPar052(par); break;	//treshold ber
case 0x53:    SetPar053(par); break;	//e
//case 0x6C:    SetPar06C(par); break;	//dm on
...
case 0x6B:    SetPar06B(par); break;	 //config file 1
...
strcpy(web_prm[0x6B].code,"07K=");	//config file1 number
...
if(buf) buf++;
 ...
 buf = strchr(buf, '&');  //
if(buf) buf++;
 ret = CheckCode(buf);			 //7K config file1  number
...
//  case	0x23: page_number = 4; parse_page_system(buf); return;	  //code 1d
 ...
 // case	0x6A: page_number = 7; parse_page_demodulator(buf); return;	  //code 1d
  case	0x50: page_number = 7; parse_page_demodulator(buf); return;	  //code 1d
...
if (*(Key + 4) == '2')
  ...
  //      sprintf(NewKey, "%5d", (unsigned)DevId.Value);  //    
        sprintf(NewKey, "%4X", (unsigned)DevId.Value);  //    
        memcpy((Key+15), NewKey, 4);               // channel 7 (P6.7)
//		memcpy((Key+15), NewKey, 5);               // channel 7 (P6.7)
       }
...
//printfp(Key);
      ...
      if (*(Key + 5) == 'e')  //02e year
 	  ...
 	  // 	printfpd("\n\r Year : %d", (unsigned)ClockReadYear());
	 ...
	 // 	printfpd("\n\r Month : %d", (unsigned)ClockReadMonth());
        memcpy((Key+15), NewKey, 2);               // channel 7 (P6.7)
...
if (*(Key + 5) == '4')  //name
  ...
  //	 	sprintf(NewKey, "%2d", (unsigned)ConfigFile.Value); 
	 	sprintf(NewKey, "%3d", (unsigned)ConfigFile.Value); 
...
// 		sprintf(NewKey, "%s", MOutLev.pResultStr);
	  ...
	  //	sprintf(NewKey, "%f3.1", MOutLev.Value *(-1));
			sprintf(NewKey, "%f4.2", MOutLev.Value *(-1)- 0.0001);
...
if (*(Key + 5) == 'K')  //name
       {
 	// 	sprintf(NewKey, "%2d", (unsigned)ConfigFile1.Value); 
	 	sprintf(NewKey, "%3d", (unsigned)ConfigFile1.Value); 
	//	NamesConfigFiles.MakeResultStrV();
    //	sprintf(NewKey, "%s", NamesConfigFiles.pResultStr);
    // 
    //      memcpy((Key+15), NewKey, 4);       
	...
	if (*(Key + 5) == 'L')  //name
       {
 	 //	sprintf(NewKey, "%2d", (unsigned)ClockReadDate()); 
		NamesConfigFiles1.MakeResultStrV();
 		sprintf(NewKey, "%s", NamesConfigFiles1.pResultStr);
        // 
      //      memcpy((Key+15), NewKey, 4);       
        memcpy((Key+15), NewKey, 15);                
       }
...
if (*(Key + 5) == 'P')  //eth rate D
       {
 	 	sprintf(NewKey, "%11d", (unsigned)MRateED.Value); 
        memcpy((Key+15), NewKey, 11);                
       }
