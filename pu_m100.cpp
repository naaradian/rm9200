...
extern "C" void PU_M()
 ...
 if(time1 - read_state_time >= READ_STATE_PERIOD) 
  {
	 read_state_time = time1;
	 ReadPum100();	//fix values
	long err_tmp_now = 0l;
...
//		unEmb2TypeVer.emb2TypeVer.signature_hardware[2] = inportb(0x514);
		...
		unEmb2TypeVer.emb2TypeVer.signature_hardware[12] = 0; //inportb(0x522);
		unEmb2TypeVer.emb2TypeVer.signature_hardware[13] = inportb(0x523);
		unEmb2TypeVer.emb2TypeVer.signature_hardware[14] = 0; //inportb(0x524);
		unEmb2TypeVer.emb2TypeVer.signature_hardware[15] = 0; //inportb(0x525);
...
//________________________________________________________________________________>> from bmd155
 ...
 //port  err_tmp = (unsigned short)inportb(0x526) + ((unsigned short)inportb(0x527)<<8);
 	/* port
   err_tmp = 0; //port
   ...
   if(err_tmp >= count_no_correct_result) 
	...
	unStateBMD155.stateBMD155.count_no_correct_result= 0;	//port
	unStatePUM.statePUM.ulErrOut = 0;	//port
	ErrResSNMP += 0; //port

//counter of received packets


 //port	err_tmp = (unsigned short)inportb(0x532) + ((unsigned short)inportb(0x533)<<8);
 /*
dev.pum100s.modeEth = inportb(MODE_ETH_ADDR);
 dev.pum100s.mode    = inportb(MODE_ADDR);
 dev.pum100s.operate= inportb(OPERATE_ADDR);
...
*/
   //	 dev.pum100s.QAM32_1 = 0x12l;
   //	  dev.pum100s.QAM32_2 = 0x34l;
   //	   dev.pum100s.QAM32_3 = 0x56l;

	 tmpcnt = dev.pum100s.QAM32_3;
	 tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.QAM32_2;
	  tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.QAM32_1;
	 
   //	  + (unsigned long)((unsigned long)dev.pum100s.QAM32_2 << 8) + (unsigned long)((unsigned long)dev.pum100s.QAM32_3 << 16); 
   	  deverr.errors100.QAM32cnt1 +=	tmpcnt;
	 //	deverr.errors100.QAM32cnt1 =	tmpcnt;
	   ...
	   err_tmp =  tmpcnt;
 //t   deverr.errors100.QAM32cnt1 += (unsigned long)dev.pum100s.QAM32_1 + (unsigned long)((unsigned long)dev.pum100s.QAM32_2 << 8) + (unsigned long)((unsigned long)dev.pum100s.QAM32_3 << 16); 
//	  deverr.errors100.QAM32cnt1 += (unsigned long)dev.pum100s.QAM32_1 + (unsigned long)(dev.pum100s.QAM32_2 << 8) + (unsigned long)(dev.pum100s.QAM32_3 << 16); 
     tmpcnt = dev.pum100s.QAM16_3;
	 tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.QAM16_2;
	 tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.QAM16_1;

 	 deverr.errors100.QAM16cnt1 +=	tmpcnt;
 	  err_tmp +=  tmpcnt;

 
 //	deverr.errors100.QAM16cnt1 += (unsigned long)dev.pum100s.QAM16_1 + (unsigned long)dev.pum100s.QAM16_2<<8 + (unsigned long)dev.pum100s.QAM16_3<<16;
 
    tmpcnt = dev.pum100s.QAM8_3;
	tmpcnt <<= 8;
	tmpcnt += dev.pum100s.QAM8_2;
	tmpcnt <<= 8;
	tmpcnt += dev.pum100s.QAM8_1;

 	 deverr.errors100.QAM8cnt1 +=	tmpcnt;
 	    err_tmp +=  tmpcnt;


   //	deverr.errors100.QAM8cnt1  += (unsigned long)dev.pum100s.QAM8_1  + (unsigned long)dev.pum100s.QAM8_2<<8  + (unsigned long)dev.pum100s.QAM8_3<<16;
 	   tmpcnt = dev.pum100s.QPSK_3;
	 tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.QPSK_2;
	  tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.QPSK_1;

 	 deverr.errors100.QPSKcnt1 +=	tmpcnt;
 	     err_tmp +=  tmpcnt;


 
   //	deverr.errors100.QPSKcnt1  += (unsigned long)dev.pum100s.QPSK_1  + (unsigned long)dev.pum100s.QPSK_2<<8  + (unsigned long)dev.pum100s.QPSK_3<<16;
 	   tmpcnt = dev.pum100s.BPSK_3;
	 tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.BPSK_2;
	  tmpcnt <<= 8;
	 tmpcnt += dev.pum100s.BPSK_1;

 	 deverr.errors100.BPSKcnt1 +=	tmpcnt;
	     err_tmp +=  tmpcnt;

 	 
 
  //	deverr.errors100.BPSKcnt1  += (unsigned long)dev.pum100s.BPSK_1  + (unsigned long)dev.pum100s.BPSK_2<<8  + (unsigned long)dev.pum100s.BPSK_3<<16;

//   printEthLongHex(deverr.errors100.QAM32cnt1);
	/*
	 err_tmp = 	(unsigned long)dev.pum100s.QAM32_1 + (unsigned long)(dev.pum100s.QAM32_2<<8) + (unsigned long)(dev.pum100s.QAM32_3<<16) +
				(unsigned long)dev.pum100s.QAM16_1 + (unsigned long)(dev.pum100s.QAM16_2<<8) + (unsigned long)(dev.pum100s.QAM16_3<<16) +
				(unsigned long)dev.pum100s.QAM8_1  + (unsigned long)(dev.pum100s.QAM8_2<<8)  + (unsigned long)(dev.pum100s.QAM8_3<<16)  +
				(unsigned long)dev.pum100s.QPSK_1  + (unsigned long)(dev.pum100s.QPSK_2<<8)  + (unsigned long)(dev.pum100s.QPSK_3<<16)  +
				(unsigned long)dev.pum100s.BPSK_1  + (unsigned long)(dev.pum100s.BPSK_2<<8)  + (unsigned long)(dev.pum100s.BPSK_3<<16);
		 */
  ...
  /* port
	...
	unStatePUM.statePUM.ulBlock = err_tmp;
  */
   unStateBMD155.stateBMD155.count_block = err_tmp;
   unStatePUM.statePUM.ulBlock +=  err_tmp;


  //do not corrected errors


  //port	err_tmp = (unsigned short)inportb(0x52E) + ((unsigned short)inportb(0x52F)<<8);

	tmpcnt = dev.pum100s.RSU2;
	  tmpcnt <<= 8;
	  tmpcnt += dev.pum100s.RSU1;
	 
	 err_tmp = 	tmpcnt;

 //	err_tmp = 	(unsigned long)dev.pum100s.RSU1 + ((unsigned long)dev.pum100s.RSU2 << 8);
  ...
  //port	err_tmp = (unsigned short)inportb(0x530) + ((unsigned short)inportb(0x531)<<8); 
   ...
   /* port
    ...
    err_tmp_prev4 = err_tmp;
 	...
 	err_tmp - count_notRS2;//count_notRS2;
	else 
	...
	unStatePUM.statePUM.ulErrAfterRS2 = err_tmp;
			 */
unStateBMD155.stateBMD155.count_notRS2 = 0;
 unStatePUM.statePUM.ulErrAfterRS2 = 0;

//corrected errors

//port	err_tmp = (unsigned short)inportb(0x528) + ((unsigned short)inportb(0x529)<<8) + ((unsigned long)inportb(0x52A)<<16);
   tmpcnt = dev.pum100s.RSC2;
	  tmpcnt <<= 8;
	  tmpcnt += dev.pum100s.RSC1;
 ...
 //	err_tmp = dev.pum100s.RSC1 + (dev.pum100s.RSC2 << 8);
   ...
   //port	err_tmp = (unsigned short)inportb(0x52B) + ((unsigned short)inportb(0x52C)<<8) + ((unsigned long)inportb(0x52D)<<16);
	/* 
	...
	unStatePUM.statePUM.ulErrBeforeRS2 = err_tmp;
	   */
	unStateBMD155.stateBMD155.count_no_correct_block2 = 0;
 	unStatePUM.statePUM.ulErrBeforeRS2 = 0;
 ...
 */
    unStatePUM.statePUM.state[0] = inportb(0x520);
	unStatePUM.statePUM.state[1] = inportb(0x521);
	unStatePUM.statePUM.state[2] = 0;
	unStatePUM.statePUM.state[3] = 0; /// Повтор
	unStatePUM.statePUM.state[4] = 0;
	unStatePUM.statePUM.state[5] = 0;
...
//		if(embRS232.UsedReceive())
		{
			...
			case 0x55: 
				...
				embRS232.Trans(0x55);
				break; 
			case 0xAA: embMsg.Init(); break; // Пакет
			...
			default:
				...
				else embMsg.Add(byte);
			}
			if(embMsg.IsEnd()) break;
			if(++i232>10) break;
		}
///////////////////////////////
///   Формирование ответа   ///
...
//							bt = (unsigned char far*)0x80000000;
			   ...
			   //		printEthLongHex(0x14);
				...
				else
				{
					if(flagRequest == 1) 
					{
						tBuffEmbMsgEast.Add(embMsg);
						tBuffEmbMsgWest.Add(embMsg);
