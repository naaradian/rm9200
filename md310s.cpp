...
//				  unsigned char interf,
 ...
 printfpd("  flagm %d",  (unsigned char) flagm);
...
//bad 120608 return;
FillMenu(); //fill items for indikator; need set it to property point
SetItemConfig();
ReadRAnyConfigs();
...
//StartLoadAllM(); 150121//temporary hieer; need in end
...
if(ver_api != (unsigned long)BAD_VALUE_ANS)	  //if have some answer - provingent is loaded - hot restart
{
 hot_restart = 1;
...
//PVG610_NET_ETH_PM_STRUCT Pm; //150126
...
//  printfpd("\n\r1: %d", Ms.acquireStatus);
 ...
 if((GetMIfLoop()) && (!Ms.acquireStatus))
 {
 //  SetMIfLoop();
...
unsigned char ans; 
 ...
 ans = PVG610_API_NetEthPmGet(0, 0, &Pm, &OneSecondPeriodCounter);
 if(ans) return;
...
// SetMEthRcvD(Pma.gfp.droppedFrames);
 //SetMEthRcvD(Pm.gfp.fcsErrors + Pm.gfp.checErrors + Pma.gfp.droppedFrames);
...
{
  //	 printfpd("\n\rn> %d", norma_status[deviceindex]);
   //	 printfpd("d: %d", deviceindex);

 //t  	norma_status[deviceindex]--;   //to not get to maximum and next step to zero
...
// printfp("\n\rans_correct!!!!");
 // printfpd("  %d", norma_status[deviceindex]);
   //	 printfpd("d: %d", deviceindex);
...
maskcnt = 1;
 ...
 UpdateDataVal();  //to redraw menu for monitoring
...
pEs.changePhyId = changephy;
 ...
 printfp("\n\r EthStatusSet1");
  printfpd (" id : %d", phyId);
    printfpd (" speed : %d", speed);
	  printfpd ("\n\r changephy: %d", changephy);
	    printfpd (" autoNegotiation: %d", auton);
		    printfpd (" duplex: %d", duplex);
