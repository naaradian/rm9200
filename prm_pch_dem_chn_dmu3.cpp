...
extern "C" void LoadBaud(unsigned char rate, unsigned char chnl)
{
 unsigned char code;
...
extern "C" void LoadBand(unsigned long freq, unsigned char chnl)
{
//unsigned char * pData;
//static unsigned char DataDDS[SENDED_DDS_FREQ_LEN];//
   SetSelector(chnl);	
		...
		if (chn <= MAX_CNL)
		{
		 ...
		 case CNL1 :  
					 ...
					 //  if(1)
						 {
		 				  ScanStateB();
   						  dmustatusu.dmustatus.mode = ReadModulSKB(CNL1);
					 ...
					 //  if(1)
						 {
		  				   ...
		  				   ScanStateB();
   						  dmustatusu2.dmustatus.mode = ReadModulSKB(CNL2);
