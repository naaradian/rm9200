...
//_______________________________________________________________232e
   ...
   //	 	printf("\n\r>>>>");
        ...
        if(byte == 0x44) //unlock
         { 
          if((lockstate != MOD_UNLOCK)&& (unlockcounter >= MAX_UNLOCK_VALUE))
          { 
          SetModUnLock();
          lockstate = MOD_UNLOCK;
         
           }
             else
          {
             if(unlockcounter < MAX_UNLOCK_VALUE)
            { 
              unlockcounter++;
            }
          }
