...
*/
//_______________________________________________________port
#include "start.h"
#include "hello7.h"
#include "embisr.h"
#include "basic.h"
...
//______________________________Need init!!!!!!!!!!!!!!!!!
//int tscount;
int plcount;
...
//	asm { db 0fah, 0eah, 0b7h,07h,00h,27h; } //версия БУ 138   //? need hier reset BU?
...
void Callback0()
{
  //090519 for not have call xilinx 	test_rb();
}

//void EmbTreeTVM::Outportb(int port, unsigned char value)
...
{
unsigned char value;
 value = GetVideoStandart()  & 0x1;
 //  spi_tst(ch);
 value += ((GetTestSignal() & 0x1) << 2);
 outportb(5 , value);
// printf("\n\r restore reg 5 : %X", value);
}
...
*/
	outportb(0x6ff,0x04);  //? need
  /* port
	outport(0xffe6,0x3333);
	outport(0xffe6,0xcccc);
	outport(0xffe6,0xc080);
	*/
/*
	ConvertToWin1251(str_k);
	ConvertToWin1251(str_v);
	ConvertToWin1251(str_e4);
	ConvertToWin1251(str_e3);
*/	
//ok return;
//_________________________
  EmbInitNVRAM(); //090226
//_________________________
  InitFreePtr(); //!!!!!!!!!!!!!!!!!!!!!!!need init before allocate memory!
  AsciiInit();
  EmbInit();
...
//	  dpd_state = 0; //temporary
  //bad	 return;
  //	embTree.SetCfg(dpd,2048); // Инициализируем дерево меню и команд управления
///////////////////////////////////////////////////////////////////////////////
	embTimer.Init(40); // *0.1 ms темп следования прерываний от таймера
	embTimerCallback0.callback = &Callback0;
	embTimerCallback0.interval = 25; 
...
//		Enable(); // Разрешаем все прерывания
	SetPll();
	Init_CS24();
	  //port 	CS24(a,c,embCommand2,embCommand3,embTree,dpd);
	CS24();
  //	}
