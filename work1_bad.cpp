	   ...
	   if(breakflag)
		   {
			  add_char_TCP_buff(TYPE_RUN_OK);
			  add_char_TCP_buff(IPBuff[1]);
	 	      add_char_signed char)COMMAND_13)	//att
	 {
	   if(breakflag)
		   {
			  add_char_TCP_buff(TYPE_RUN_OK);
				  ...
				  case 1:  		add_char_TCP_buff(TYPE_RUN_32);
			  			  ...
			  			  //		add_char_TCP_buff(READ_ATT_COMMAND);
								...
								add_char_TCP_buff(ReadAtt1[2]);
				   	 			break;
				  case 2:  		add_char_TCP_buff(TYPE_RUN_32);
			  					add_char_TCP_buCP_buff(ReadAtt3[1]);
	   ...
	   if(breakflag)
		   {
			  switch(IPBuff[1])
			  {
				  case 1:  if(WriteStatus1)
				   			{
			 	 				add_char_TCP_buff(TYPE_RUN_OK);
			  					add_char_TCP_buff(IP				add_char_TCP_buff(WRITE_FLASH_COMMAND);
				   			}
				   			else
				   			{
				    			add_char_TCP_buff(TYPE_RUN_BAD);
...
if((unsigned char)IPBuff[0] == (unsigned char)READ_ATT_COMMAND)	//read att
 ...
 //		   add_char_TCP_buff(IPBuf				 				add_char_TCP_buff(LOST_CODE);		  //can be changed
 ...
 //		   add_char_TCP_buff(IPBufufferState((unsigned long)IP_BUFFER_OVERFLOW);
		}
	 	buffer_rad_T[buffer_rad_T_len++] = tch;
...
if((unsigned char)IPBuff[0] == (unsigned char)COMMAND_13)	//att
 ...
 //		   add_char_TCP_buP_buff(IPBuff[1]);
			  					add_char_TCP_buff(WRITE_FLASH_COMMAND);
				   			}
				   			else
				   			{
...
extern "C"unsigned long GetNetMaskRem(vo; //some load
				WriteDevId();
}
...
extern "C"unsigned long GetNetMaskRem(voASE;
...
if(!mod)
{
  ClearBufferState((unsigned long)OPERATE_POWERit - operate power
extern "C" void SetModForSS1(unsigned long mod)
{
//printfpd("\n\rmod%x",mod);
// ModForSS.Value = (float64)mod;
...
if(!mod)
{
  ClearBufferState((unsigned long)OPERATE_POWER);
...
typedef struct _AT91S_SPI {
	...
	unsigned long	 SPI_TDR;__8 modems
 case 8 : Threshold8.Value  = val;	 break;
...
//	}
 ...
 // 	AT91C_BASE_SPI->SPI_PTCR = AT91C_PDC_TXTDIS;
 //  AT91C_BASE_SPI->SPI_MR &= 0xFFF0FFFF;   //csoff
 (!(AT91C_BASE_SPI->SPI_SR & AT91C_SPI_RXBUFF));
...
//		test
   ...
   //			AT91C_BASE_SPI->SPI_MR |=0x33, 0x11};
...
//		test
		...
		AT91C_BASE_SPI->SPI_MR &= 0xFFF0FFFF;
   		AT91C_BASE_SPI->SPI_MR |= ((AT91C_SPI_PCS2_SERIAL_DATAFLASH<<16) & AT91C_SPI_PCS);
   //			AT91C_BASE_SPI->SPI_MR |=	AT91C_BASE_SPI->SPI_TPR = pData; //(unsigned int)pDesc->tx_cmd_pt ;
...
//#endif
		...
		if(addr_dest  =  (unsigned int)&TFTP_Buffer1[0]; //use tftpbuffer1 to do not broken tftpbuffer
  //	   write_dataflash (addr_dest, (unsigned int)&TFTP_Buffer1[0], 256);
...
extern "C" void CheckOperateFlashx(void)
{
 ...
 unsigned flag = 1;
  ...
  write_dataflashx(ADEST, (unsigned long)&BuffW[0] , SIZEX);
 //  printfp("\n\r set : ");
