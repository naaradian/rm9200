...
extern "C" void LoadSPIDevR(unsigned char cs, unsigned char data, unsigned char bitsval)
{
  AT91RM9200_PIO_REG_STRUCT_PTR  pio_ptr;
  pio_ptr = (AT91RM9200_PIO_REG_STRUCT_PTR) AT91RM9200_PIOB_BASE;
//  unsigned char mask = 1;
  ...
  // pio_ptr->SODR = MASK_BIT_14 | MASK_BIT_13 ;
 //170327  pio_ptr->SODR = MASK_BIT_13 ;
   delay_mcs(2);
...
extern "C" unsigned char ReadSPIDevR(unsigned char cs,  unsigned char bitsval)
{
  AT91RM9200_PIO_REG_STRUCT_PTR  pio_ptr;
  pio_ptr = (AT91RM9200_PIO_REG_STRUCT_PTR) AT91RM9200_PIOB_BASE;
  unsigned char mask = 0x80;
...
extern "C" unsigned char IOSpiSendR(unsigned char cs, 
...
//  unsigned char mask = 0x80;
   ...
   _at91rm9200_pio_set(AT91RM9200_PIO_PORT_B, 11, AT91RM9200_PIO_PERIPHERAL_IO, 1);	  //le4	dac
   _at91rm9200_pio_set(AT91RM9200_PIO_PORT_C, 14, AT91RM9200_PIO_PERIPHERAL_IO, 1);	  //le6	adc
   _at91rm9200_pio_set(AT91RM9200_PIO_PORT_C,  2, AT91RM9200_PIO_PERIPHERAL_IO, 1);	  //	adc1


   pio_ptr->SODR = MASK_BIT_19 |  MASK_BIT_18 |MASK_BIT_17 |MASK_BIT_14 | MASK_BIT_13 | MASK_BIT_15 | MASK_BIT_11;
   pio_ptra->SODR = MASK_BIT_19;
//   pio_ptrc->SODR = MASK_BIT_3 | MASK_BIT_14;
   pio_ptrc->SODR = MASK_BIT_3 | MASK_BIT_14 | MASK_BIT_2;
...
switch (cs)
{
  ...
  case 6 : 	  pio_ptr->CODR  = MASK_BIT_11; break;	 //dac
  case 7 : 	  pio_ptrc->CODR = MASK_BIT_14; break;	 //adc
  case 8 : 	  pio_ptrc->CODR = MASK_BIT_2; break;	 //adc1
 ...
 //171023  printfpd(" %02X",(unsigned char)(* BufferData));
 ...
 switch (cs)
{
  ...
  case 6 : 	  pio_ptr->SODR  = MASK_BIT_11; break;	 //dac
  case 7 : 	  pio_ptrc->SODR = MASK_BIT_14; break;	 //adc	 
  case 8 : 	  pio_ptrc->SODR = MASK_BIT_2; break;	 //adc1
...
IOSpiSendR(chnl, 2, 0, BufferData, BufferData);
...
Buff[1] = 0x80;
if(baud > 31)  Buff[1] |= 0x40;

if(baud & 0x1)	 Buff[1] |= 0x20;
if(baud & 0x2)	 Buff[1] |= 0x10;
if(baud & 0x4)	 Buff[1] |= 0x8;
if(baud & 0x8)	 Buff[1] |= 0x4;
if(baud & 0x10)	 Buff[1] |= 0x2;
if(baud & 0x20)	 Buff[1] |= 0x1;
	 
/*
Buff[1] = 0x80;
if(baud > 31)  Buff[1] |= 0x40;
  
if(baud & 0x1)	 Buff[1] |= 0x10;
if(baud & 0x2)	 Buff[1] |= 0x8;
if(baud & 0x4)	 Buff[1] |= 0x4;
if(baud & 0x8)	 Buff[1] |= 0x2;
if(baud & 0x10)	 Buff[1] |= 0x1;
//if(baud & 0x20)	 Buff[1] |= 0x2;
 */
//printfpd("\n\r load baud : %d", baud);
...
if(LNB15.Value > 1)
{
   LNB15.Value = 0;   LNB15.ChangingValue = 0;
}
  
if(LNB10.Value > 1)
{
   LNB10.Value = 0;   LNB10.ChangingValue = 0;
}

if(BUC24.Value > 1)
{
   BUC24.Value = 0;   BUC24.ChangingValue = 0;
}

if(BUC10.Value > 1)
{
   BUC10.Value = 0;   BUC10.ChangingValue = 0;
}
...
else if(tmp < 250000){ SetAny(1, 12, tmp3,100l   ,3l);}
else if(tmp < 450000){ SetAny(1, 0, tmp3,200l    ,3l);}
else if(tmp < 600000){ SetAny(1, 0, tmp3,300l    ,3l);}
...
LoadIFDAC((unsigned long) ((SatSet4.Value + SatSet8.Value) * MULT_VALF));

 printfp("SetTrFrequency");
#ifndef PROG_PU_MSAT5	
 LoadDDSN3a();
#else
//170816 LoadIFMd((unsigned long) ((SatSet4.Value + SatSet8.Value) * MULT_VALF));
if(SatTiming.Value)
{
   printfpd("SetTrFrequency invert %d", SatTiming.Value);

 LoadIFDmd((unsigned long)((SatSet4.Value - SatSet8.Value) * MULT_VALF));	 //spectrum invert
 }
else
{
 printfp("SetTrFrequency normal");

 LoadIFDmd((unsigned long)((SatSet4.Value + SatSet8.Value) * MULT_VALF));	 //170816
 }
...
{
LoadIFADC((unsigned long)((SatSet5.Value + SatSet8.Value) * MULT_VALF));
// printfp("SetRcvFrequency");
#ifndef PROG_PU_MSAT5	
 LoadDDSN3();
#else
 LoadIFDmd1((unsigned long)((SatSet5.Value + SatSet8.Value) * MULT_VALF));
 LoadIFMd((unsigned long) ((SatSet5.Value + SatSet8.Value) * MULT_VALF));	//170816
...
{
   return (unsigned char)BUC24.Value;
}
...
extern "C" void SetBUC10M(void)
{
   AT91RM9200_PIO_REG_STRUCT_PTR pio_ptrc;
   pio_ptrc =  (AT91RM9200_PIO_REG_STRUCT_PTR) AT91RM9200_PIOC_BASE;
   _at91rm9200_pio_set(AT91RM9200_PIO_PORT_C, 0, AT91RM9200_PIO_PERIPHERAL_IO, 1);
...
{
   return (unsigned char)BUC10.Value;
}
...
extern "C" void SetLNB15V(void)
{
   ...
   _at91rm9200_pio_set(AT91RM9200_PIO_PORT_B, 22, AT91RM9200_PIO_PERIPHERAL_IO, 1);
    ...
    printfpd("\n\r Lnb15 v :%d", (unsigned)LNB15.Value);
	 printfpd("cv :%d", (unsigned)LNB15.ChangingValue);


 if(LNB15.ChangingValue > 0)
 {
   printfp("\n\r Set Lnb15V:");
 	 pio_ptrb->CODR = 1<<22; 
 }
...
{
   return (unsigned char)LNB15.Value;
}
...
extern "C" void SetLNB10M(void)
{
   AT91RM9200_PIO_REG_STRUCT_PTR pio_ptrc;
   pio_ptrc =  (AT91RM9200_PIO_REG_STRUCT_PTR) AT91RM9200_PIOC_BASE;
   _at91rm9200_pio_set(AT91RM9200_PIO_PORT_C, 1, AT91RM9200_PIO_PERIPHERAL_IO, 1);
...
{
   return (unsigned char)LNB10.Value;
}
