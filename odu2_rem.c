...
//	  printEthStringAdd(2,0);
...
#ifdef IGNORE_AA_IN_5
              
             // if(embMsg485.counter == 5)
              if((embMsg485.counter == 5) || (embMsg485.counter == 3))
              {
                       embMsg485.Add(byte);
               if(GetTestMode() == 0x50)
			            {
                        printfpd(" (cnt:%d )",  embMsg485.counter);
                     }               
              } 
              else
              { 
             
            mode5A485=0;  embMsg485.Init(); stemp++;
               }
#else
             mode5A485=0;  embMsg485.Init(); stemp++;
 ...
 //160906			if(embMsg485.IsEnd())
        	if(embMsg485.counter >= 34) //160906
			{
  //			  printfpd("\n\r length + 2 : %d ",  embMsg485.Length()+2);
...
//__________________________test  !!!
				...
				break; 
			case 0xAA: embMsg232.Init();  mode5A=0; break; // Iaeao
...
//160603		tBuffRS485.Add(*embMsgRequest);   //send to edik request - wrong - need ti rs485e!!!
     ...
     printfpd(" %04X ", embMsgRequest->AddrS());                         
...
//printEthLongHex(4); 
 ...
 //  printEthLongHex(4);
    ...
    #ifdef ETH_TRANZIT   
 // 160826  #ifdef MAKET_STEND
                   tBuffEmbMsgEthAll.Add(embMsg232);     //for test!!!!!!!!!!!!!!!!!!
                   
 //   #endif               
    #endif     
                 
#ifdef DEBUG_ETH_TRANZIT
        //               tBuffEmbMsgEthAll.Add(embMsg232);  //hier only without stend
#endif 


     				
//#ifdef DEBUG_MAKET    
  //        tBuffEmbMsgEthAll.Add(embMsg232);   //temporary
//#endif           
...
//___________________________________________________________________________________
 ...
 //   #ifdef ETH_TRANZIT   
                         tBuffEmbMsgEthAll.Add(embMsg485E);   //to transit channel
 //   #endif                       
...
#include stack_commands_odu.c // rBuffEmbMsg
 ...
 //________________________________________________________________________________________RDN
  ...
  //============================================================================================		
   ...
   #ifdef ETH_TRANZIT   
        //        tBuffEmbMsgEthAll.Add(* embMsgRequest);
   #endif         
   //  need add eth_tranzit buffer
		//				unEmb2NetStat.emb2NetStat.selfPacketSend++;
			 //	}	// == 2
...
#include stack_commands_odu.c // rBuffEmbMsg
				 ...
				 //	 printf(",%1X",flagRequest);
  				  ...
  				  //	printEthLongHex(flagRequest);	 //t
 //160630              tBuffEmbMsgEast.Add(embMsgAns);
...
//#endif            
		  		...
		  		//  		tBuffEmbMsgEast.Add(*embMsgRequest);
	  //transit not need			  		tBuffEmbMsgWest.Add(embMsgAns);
...
//#ifdef RS485_MSG_BUFF
 ...
 //    printfpd("\n\r%d> ", time1);                    
 //    printfpd("flag : %04X ", flag_wait_rs485E);                         
 //   OperateBuffers_usart0t();
#endif     
...
//#ifdef RS485E_ENABLED
 ...
 //#endif          
     ...
     printfpd("\n\r%d> start trans ", time1);                    
     OperateBuffers_usart0t();
#endif     
             
                     //    delay_mcs(10000);
  	 					   	embRS485.Trans_RDN(embMsgA.AddrS());   //hier hangt!!!!
                        //  delay_mcs(100);
     ...
     OperateBuffers_usart0t();
					...
					buff_udpt_485[l_udpt_inc_485()] =	0xAA;
					for(i=0; i<embMsgA.FullSize(); i++) 
						{
				 			switch(embMsgA.body[i])
							{
								case 0x5A:   buff_udpt_485[l_udpt_inc_485()] = 0x5A; buff_udpt_485[l_udpt_inc_485()] = 0x00; break;
								case 0x55:  buff_udpt_485[l_udpt_inc_485()] =	0x5A;  buff_udpt_485[l_udpt_inc_485()] = 0x01; break;
								case 0xA5: buff_udpt_485[l_udpt_inc_485()] =	0x5A; buff_udpt_485[l_udpt_inc_485()] = 0x02; break;
								case 0xAA: buff_udpt_485[l_udpt_inc_485()] =	0x5A; buff_udpt_485[l_udpt_inc_485()] = 0x03; break;
								default: buff_udpt_485[l_udpt_inc_485()] = embMsgA.body[i]; 
							}
						}

						 buff_udpt_485[l_udpt_inc_485()] = 0x55;
						 flag_send_udp_asu_485 = 1; //can send it
