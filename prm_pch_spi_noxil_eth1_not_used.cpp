...
{
unsigned char * pData;
static unsigned char DataDDS[LEN_DATA_DDS + 1];//
unsigned char tmp;
//tmp =  SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]);
// for(int i = 0; i < 1000l; i++)
// {
  pData = DataDDS;
 *pData++ = 0xe0 + (0x1f & (addr>>8));   *pData = (addr & 0xff);	pData++; *pData = 0xff;  *pData++ = 0;
 tmp =  SpiReadN2 (LEN_DATA_DDS + 1, (unsigned long)&DataDDS[0]);	 //+1: clocks for readed byte
 printfpd("\n\r %x : ",addr);
 printfpd(" %x ",tmp);
 //wd_reset();
// }
...
//tmp =  SpiReadN2 (LEN_DATA_DDS , (unsigned long)&DataDDS[0]);
 ...
 {
 pData = DataDDS;
 *pData++ = 0xe0;   *pData++ = i;	 *pData++ = 0xff;  *pData++ = 0x88;
...
//*pData++ = 0;   *pData++ = 0;	 *pData = 0x99;	  //make sdo
*pData++ = 0;   *pData++ = 0;	 *pData = 0x88;	  //make sdo
SendToDDS(LEN_DATA_DDS,  (unsigned long)&DataDDS[0]);
