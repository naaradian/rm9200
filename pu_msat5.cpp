...
pData = DataDDS;
*pData++ = 0x42;   *pData++ = 0;	 *pData = 0x08;//0x09;  //addr 21
IOSpiSendR(3  , 3, 0,  DataDDS, DataDDS);	  //channel D
...
pData = DataDDS;
*pData++ = 0x42;   *pData++ = 0;	 *pData = 0x09; //fref 40 //0x09;  //addr 21
IOSpiSendR(4   , 3,  0,DataDDS, DataDDS);	  //channel C
IOSpiSendR(5   , 3,  0,DataDDS, DataDDS);	
...
//#define FREQ_ODIV  (1060)

#define FREQ_REF  (20000000) // (80000000) // (10000000)	
#define FREQ_REFD  (40000000)
...
{
unsigned long i;
printfpd("freq : %d", freq);
freq /= 1000000l;
for(i = 1; i < DKOEF_SIZE; i++)
{
...
unsigned char ret = 0;
// printfpd("\n\r getdreg i : %d ", i);
// printfpd("\n\r dkoef[i].rdac << 5: %02X ", dkoef[i].rdac <<5);
// printfpd("\n\r dkoef[i].cdac: %02X ", dkoef[i].cdac);
...
case 602: ret = (dkoef[i].cout << 5) + (dkoef[i].cin << 1);	 break;
case 621: ret = (dkoef[i].bias << 2) +  (dkoef[i].rdac >> 3);  	break;
case 622: ret = (dkoef[i].rdac << 5) + dkoef[i].cdac; 	break;
...
switch(r)
{
case 602: ret = (mkoef[i].cout << 4) + mkoef[i].cin ;	break;
case 621: ret = mkoef[i].rsel >> 2;	break;
case 622: ret = (mkoef[i].rsel << 6) + mkoef[i].csel;	break;
...
if((freq / 1000000)> (FREQ_ODIV)) { DIV4_EN = 0; Fvco = (freq/100) << 1; }
//else { DIV4_EN = 1; Fvco = (long long)(freq << 2);}
else { DIV4_EN = 1; Fvco = (freq /100) << 2;}
...
;
INT_K  = Fvco/ ((unsigned long)(FREQ_REFD<<1) / 100l);

//frac = ((Fvco % (long long)(FREQ_REF<<1)) << 16) / (unsigned long)(FREQ_REF<<1);
frac = ((Fvco % ((FREQ_REFD<<1) / 100)) << 16) / ((unsigned long)(FREQ_REFD<<1) / 100);

printfpd("\n\rD INT_K : %d \n\r", INT_K);
printfpd(" D frac : %d ", frac);
printfpd(" Fvco / 100 : %d ", Fvco);
...
pData = DataDDS;
*pData++ = 0x42;   *pData++ = 0;	 *pData = 0x08; //170619 //0x29;//0x28; //0x09;  //addr 21
IOSpiSendR(4   , 3,  0,DataDDS, DataDDS);	  //channel C
//IOSpiSendR(5   , 3,  0,DataDDS, DataDDS);	
...
//________________________________uncomment
pData = DataDDS;
*pData++ = 0x60;   *pData++ = 0;	 *pData = GetDReg (i,602);  //addr 30
//printfpd("\n\r 602 need 22 : %02X ", GetDReg (i,602));

IOSpiSendR(4   , 3,  0,DataDDS, DataDDS);	  //channel C
//IOSpiSendR(5   , 3,  0,DataDDS, DataDDS);

//printfpd("\n\r 621 need 5 : %02X ", GetDReg (i,621));
//printfpd("\n\r 622 need 5 : %02X ", GetDReg (i,622));
...
*pData++ = 0x44;   *pData++ = 0x2A;	    *pData = (DIV4_EN << 3) | VCO_SEL;  // (VCO_SEL << 1) | DIV4_EN;


printfpd("\n\r 44 : %02X ", (DIV4_EN << 3) | VCO_SEL);
...
//printfpd("\n\r 4 need 8 : %02X ",  (unsigned char)(INT_K >> 8));
...
// printfpd("\n\r 6 need 8 : %02X ",(unsigned char)(frac >> 8));
//printfpd("\n\r 6-1 need 8 : %02X ",(unsigned char)frac);
...
if((freq / 1000000)> FREQ_ODIV) { DIV4_EN = 0; Fvco = freq << 1; }
...
//_________--------t
pData = DataDDS;
*pData++ = 0x60;   *pData++ = 0;	 *pData = 0x22;  //addr 30//addr 30R(4   , 3,  0,DataDDS, DataDDS);	  //channel C
IOSpiSendR(5   , 3,  0,DataDDS, DataDDS);	
...
;
INT_K  = Fvco/ ((unsigned long)(FREQ_REF<<1) / 100l);

//frac = ((Fvco % (long long)(FREQ_REF<<1)) << 16) / (unsigned long)(FREQ_REF<<1);
frac = ((Fvco % ((FREQ_REF<<1) / 100)) << 16) / ((unsigned long)(FREQ_REF<<1) / 100);
...
if((freq /1000000l) > FREQ_ODIV) { DIV4_EN = 0; Fvco = freq << 1; odiv = 1; }
...
pData = DataDDS;
*pData++ = 0x42;   *pData++ = 0;	 *pData = 0x09;//0x29;//t 0x08;  //addr 21
IOSpiSendR(3  , 3,  0,  DataDDS, DataDDS);	  //channel D
