...
//#endif                         
				...
				embRS232S.Trans2EC(0x55);	  //EC - RDN!!! in all
			  	embRS232S.Trans2EC(0xAA);
				for(i=0; i<embMsgA.FullSize(); i++) 
							{
								switch(embMsgA.body[i])
								{
								case 0x5A: embRS232S.Trans2EC(0x5A); embRS232S.Trans2EC(0x00); break;
								case 0x55: embRS232S.Trans2EC(0x5A); embRS232S.Trans2EC(0x01); break;
								case 0xA5: embRS232S.Trans2EC(0x5A); embRS232S.Trans2EC(0x02); break;
								case 0xAA: embRS232S.Trans2EC(0x5A); embRS232S.Trans2EC(0x03); break;
								default:
                        { 
                         embRS232S.Trans2EC(embMsgA.body[i]);
//#ifdef KSS_PRINT_LOG                         
...
//#endif                         
							...
							embRS232S.Trans2EC(0x55);
                     embRS232S.Trans_RDN(embMsgA.AddrS());
  //                   embRS232S.Trans_RDN_Brdcst();
//#ifdef KSS_PRINT_LOG                         
//                         printfpd(" addrS :%X", embMsgA.AddrS());
//#endif       
...
//#endif                     
							...
							for(i=0; i<embMsg232S.FullSize(); i++) 
							{
								...
								default: embRS232.TransEC(embMsg232S.body[i]);
                  //      printfpd("*%02X*",embMsg232S.body[i]);
								}
							}
							embRS232.TransEC(0x55);
                     embRS232.Trans_RDN(embMsg232S.AddrS());
...
//========================================================================            
//========================================================         
			    if((!flagRequest) && rBuffEmbMsgNorth.Used())
				{
#ifdef FIND_CONF_ERR
		printfp("3");
#endif                
...
//========================================================================            
//========================================================         
			   if((!flagRequest)  && rBuffEmbMsgSouth.Used())
				{
#ifdef FIND_CONF_ERR
		printfp("4");
#endif                
...
//____________________________________________________________________________________RDN  <<
		...
		//100118			   flagRequest = 0;	 //t	   //new! 21.02.2008
          ...
          //23     printfpd("\n\n\rreq> %d \n",usart0_request_cnt);//ok
            //ok     printEthLongHex(8);
			  ...
			  // 			printfpd("\n\r     embMsgRequest->Type:%02X ",embMsgRequest->Type());
	...
	case 0x81 :
                 //    printfp("_______________81");
                 //     SetRelayq(embMsgRequest->Body(1));
						   embMsgAns.SetType(0x81);
                     embMsgAns.SetLength(1);
                     embMsgAns.SetBody(0, SetRelayq(embMsgRequest->Body(3)));
                  						     
                     break;
                     
   case 0x82 :
                //      printfp("_______________82");

						   embMsgAns.SetType(0x82);
                     embMsgAns.SetLength(1);
                     embMsgAns.SetBody(0, SetDir());                   						     
                     break;
...
//need make to do not answer now 
    ...
    break;   
     	...
     	case 0x57 :
                     ...
                     embMsgAns.SetLength(1);
                     embMsgAns.SetBody(0, SetDirDiff(embMsgRequest->Body(0),(char *) embMsgRequest->Body(1)));
